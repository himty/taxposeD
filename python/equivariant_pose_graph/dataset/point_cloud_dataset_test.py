...
class TestPointCloudDataset(Dataset):
    def __init__(self, dataset_root,
        ...
        # identity has higher priority than overfit
        ...
        self.filenames = [
            self.dataset_root / f'{idx}_{self.cloud_type_init}_obj_points.npz' for idx in sorted(self.dataset_indices) if idx not in self.bad_demo_id]

        if self.overfit:
            self.filenames = self.filenames[0:1]
        shapenet_id = np.load(self.filenames[0], allow_pickle=True)[
            'shapenet_id']
    ...
    def get_fixed_transforms(self):
        ...
        if self.overfit:
            ...
            for i in range(self.num_overfit_transforms):
                ...
                print("uh why is the TestPointCloudDataset being loaded? Also, Y transforms aren't implemented in this")
        elif self.init_distribution_tranform_file is not None:
            transform_init2place = np.load(self.init_distribution_tranform_file, allow_pickle=True)[
                "rack_relative_pose_stacked"]  # 100,4,4

            for i in range(len(transform_init2place)):
                tmp = random_se3(1, rot_var=self.rot_var,
                                 trans_var=self.trans_var, device=points_action.device)
                bm = torch.transpose(torch.from_numpy(
                    transform_init2place[i]), -1, -2).unsqueeze(0).float()
    ...
    def load_data(self, filename, action_class, anchor_class):
        ...
        if(action_class == 2) and self.gripper_lr_label:
            action_center = torch.from_numpy(
                point_data['gripper_pose'][0] - points_action_mean_np).view(1, 1, 3)
            action_vec = torch.from_numpy(
                point_data['gripper_pose'][2][:, 1]).view(1, 1, 3)
            ...
            symmetric_cls = torch.sign(torch.matmul(
                action_centered, action_vec.transpose(-1, -2))) == torch.sign(torch.matmul(anchor_vec, action_vec.transpose(-1, -2)))
    ...
    def __getitem__(self, index):
        ...
        if self.symmetric_class is not None:
            ...
            T_flip = rotation_se3(
                1, axis=z_axis, angle_degree=self.angle_degree, device=points_action.device)
            if self.symmetric_class == self.action_class:
                T_trans_centered = pure_translation_se3(
                    1, t=-points_action.squeeze(0).mean(axis=0), device=points_action.device)
                T_trans_uncentered = pure_translation_se3(
                    1, t=points_action.squeeze(0).mean(axis=0), device=points_action.device)
                points_action_alternative = T_trans_uncentered.transform_points(
                    T_flip.transform_points(T_trans_centered.transform_points(points_action)))
                ...
                T_trans_centered = pure_translation_se3(
                    1, t=-points_anchor.squeeze(0).mean(axis=0), device=points_action.device)
                T_trans_uncentered = pure_translation_se3(
                    1, t=points_anchor.squeeze(0).mean(axis=0), device=points_action.device)
                points_action_alternative = points_action
                points_anchor_alternative = T_trans_uncentered.transform_points(
                    T_flip.transform_points(T_trans_centered.transform_points(points_anchor)))
        ...
        if self.symmetric_class is not None:
            points_action_trans_alt = T0.transform_points(
                points_action_alternative)
            points_anchor_trans_alt = T1.transform_points(
                points_anchor_alternative)
            if self.symmetric_class == self.action_class:
        ...
        if self.no_transform_applied or self.cloud_type == 'init':
            ...
            if self.symmetric_class is not None:
                data['points_action_trans_alt'] = points_action_trans_alt.squeeze(
                    0)
                data['points_anchor_trans_alt'] = points_anchor_trans_alt.squeeze(
                    0)
                data['before_flip_original'] = before_flip_original.squeeze(0)
                data['after_flip_original'] = after_flip_original.squeeze(0)
                data['before_flip_transformed'] = before_flip_transformed.squeeze(
                    0)
                data['after_flip_transformed'] = after_flip_transformed.squeeze(
                    0)
