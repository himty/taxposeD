...
class PointCloudDataset(Dataset):
    ...
    def get_fixed_distractor_transforms(self):
        ...
        for i in range(max(self.num_overfit_transforms, self.num_demo if self.num_demo is not None else 1)):
            ...
            if self.distractor_anchor_aug:
                ...
                self.T_aug_list.append(T_aug_list)
                self.new_points_list.append(new_points_list)
    ...
    def load_data(self, filename, action_class, anchor_class):
        ...
        if self.return_rpdiff_mesh_files:
            ...
            # First 2 are for book bookshelf
            for key in ['multi_obj_mesh_file', 'multi_obj_final_obj_pose', 'ids', 
                        'selected_demo', 'selected_demo_rack', 'final_poses', 
                        'demo_rack_idx', 'scene_rack_transforms']:
                if key in point_data.files:
                    misc_point_data[key] = point_data[key]
    ...
    def downsample_pcd(self, points, type="fps"):
        if re.match(r"^fps$", type) is not None:
            ...
            random_num_points = int(re.match(r"^([0-9]+)N_random_fps$", type).group(1)) * self.num_points
            random_idx = torch.randperm(points.shape[1])[:random_num_points]
            random_points = points[:, random_idx]
            return sample_farthest_points(random_points, K=self.num_points, random_start_point=True)
        else:
    ...
    def get_data_index(self, index):
        ...
        if self.distractor_anchor_aug:
            if self.overfit_distactor_aug:
                ...
                new_points_list = self.new_points_list[index % len(self.new_points_list)]
        ...
        if(points_action.shape[1] > self.num_points):
            ...
            return self.get_data_index(index+1)
        ...
        if self.return_rpdiff_mesh_files:
            ...
            for key in ['multi_obj_mesh_file', 'multi_obj_final_obj_pose', 'ids', 
                        'final_poses', 'scene_rack_transforms', 'demo_rack_idx']:
                if key in point_data.keys():
                    data[key] = point_data[key].item()
