from equivariant_pose_graph.dataset.ndf_dataset import JointOccTrainDataset
from torch.utils.data import DataLoader
import numpy as np
from equivariant_pose_graph.dataset.pretraining_point_cloud_dataset import PretrainingPointCloudDataset
...
class PretrainingMultiviewDataModule(pl.LightningDataModule):
    ...
    def update_dataset(self):
        if self.obj_class != 'non_mug':
            ...
            self.train_dataset = PretrainingPointCloudDataset(dataset_root=self.dataset_root,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'val' or stage is None:
            ...
            if self.obj_class != 'non_mug':
                ...
                self.val_dataset = PretrainingPointCloudDataset(dataset_root=self.dataset_root,
                                                                dataset_indices=self.dataset_indices,
                                                                cloud_type=self.cloud_type,
                                                                action_class=self.cloud_class,
                                                                num_points=self.num_points)
        if stage == 'test':
            if self.obj_class != 'non_mug':
                ...
                self.test_dataset = PretrainingPointCloudDataset(dataset_root=self.dataset_root,
                                                                 dataset_indices=self.dataset_indices,
                                                                 cloud_type=self.cloud_type,
                                                                 action_class=self.cloud_class,
                                                                 num_points=self.num_points)
