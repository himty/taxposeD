...
from equivariant_pose_graph.utils.emb_losses import compute_infonce_loss, compute_occlusion_infonce_loss
from equivariant_pose_graph.utils.error_metrics import get_2rack_errors, get_all_sample_errors, get_rpdiff_min_errors
...
class EquivarianceTrainingModule(PointCloudTrainingModule):
    ...
    def __init__(self,
        ...
        self.spatial_distance_regularization_weight=spatial_distance_regularization_weight
    ...
    def get_transform(self, points_trans_action, points_trans_anchor, points_onetrans_action=None, points_onetrans_anchor=None, mode="forward", sampling_method="gumbel", n_samples=1):
    ...
    def predict(self, model_output, points_trans_action, points_trans_anchor):
        ...
        if self.decoder_type == "taxpose":
            ...
            points_trans_action_in_aligned_frame = points_trans_action - action_center.permute(0, 2, 1)
            points_action_pred_in_aligned_frame = pred_T_action.transform_points(points_trans_action_in_aligned_frame)
            pred_points_action = points_action_pred_in_aligned_frame + anchor_center.permute(0, 2, 1)
        else:
    ...
    def compute_loss(self, model_output, batch, log_values={}, loss_prefix='', heads=None):
        ...
        T_aug = Transform3d(matrix=batch['T_aug']) if 'T_aug' in batch else None
        ...
        if self.flow_supervision == "both":
            ...
            if self.decoder_type in ["taxpose"]:
                ...
                points_trans_action_in_aligned_frame = torch.cat( # action points centered about sampled action p
                    [
                        points_trans_action[:, :, :3] - action_center.permute(0, 2, 1),
                        points_trans_action[:, :, 3:]
                    ],
                    dim=-1
                )
                points_trans_anchor_in_aligned_frame = torch.cat( # anchor points centered about sampled anchor p'
                    [
                        points_trans_anchor[:, :, :3] - anchor_center.permute(0, 2, 1),
                ...
                gt_T_action = T0.inverse().compose(T1)
                points_action_target = T1.transform_points(points_action) # Mug in goal position on the transformed anchor
                
                points_action_target_in_aligned_frame = torch.cat( # Goal action points centered about sampled anchor p'
                    [
                        points_action_target[:, :, :3] - anchor_center.permute(0, 2, 1),
                ...
                gt_T_action_in_aligned_frame = translate_to_action_center.inverse().compose(gt_T_action).compose(translate_to_anchor_center)
                ...
                points_anchor_target_in_aligned_frame = torch.cat( # Goal anchor points centered about sampled action p
                    [
                        points_anchor_target[:, :, :3] - action_center.permute(0, 2, 1),
                ...
                points_anchor_pred_in_aligned_frame = pred_T_anchor.transform_points(points_trans_anchor_in_aligned_frame)
                ...
                # Loss associated flow vectors matching a consistent rigid transform
                gt_T_anchor_in_aligned_frame = translate_to_anchor_center.inverse().compose(gt_T_anchor).compose(translate_to_action_center)
        ...
        if not self.min_err_across_racks_debug:
            ...
            action_center = None
            anchor_center = None
            if self.decoder_type in ["flow", "point"] and self.flow_frame == "aligned":
            ...
            if self.compute_rpdiff_min_errors:
                coverage_error_t_mean, coverage_error_R_mean = get_rpdiff_min_errors(
                    pred_T_action, 
                    batch, 
                    rpdiff_descriptions_path=self.rpdiff_descriptions_path,
                )
                
                log_values[loss_prefix+'coverage_error_t_mean'] = coverage_error_t_mean
                log_values[loss_prefix+'coverage_error_R_mean'] = coverage_error_R_mean
        ...
        if self.model.conditioning in ["uniform_prior_pos_delta_l2norm"]:
            ...
            action_distance_prior = model_output['dense_trans_pt_action']
            anchor_distance_prior = model_output['dense_trans_pt_anchor']
            
            # Map to [0,1]
            action_distance_prior = (action_distance_prior - action_distance_prior.min()) / (action_distance_prior.max() - action_distance_prior.min())
            anchor_distance_prior = (anchor_distance_prior - anchor_distance_prior.min()) / (anchor_distance_prior.max() - anchor_distance_prior.min())
            ...
            action_kl = F.kl_div(F.log_softmax(action_distance_prior, dim=-1),
                                            F.log_softmax(goal_emb[:, :, :N], dim=-1), log_target=True,
                                            reduction='batchmean')
            anchor_kl = F.kl_div(F.log_softmax(anchor_distance_prior, dim=-1),
                                            F.log_softmax(goal_emb[:, :, N:], dim=-1), log_target=True,
                                            reduction='batchmean')
            vae_reg_loss = action_kl + anchor_kl
            loss += self.vae_reg_loss_weight * vae_reg_loss
        ...
        if heads is not None or self.model.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
            ...
            elif self.model.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                if self.hybrid_cond_regularize_all:
                    # Regularize all per-point latents to be standard normal, not just the selected ones
                    goal_emb_mu_action = model_output['goal_emb_mu_action']
                    goal_emb_logvar_action = model_output['goal_emb_logvar_action']
                    goal_emb_mu_anchor = model_output['goal_emb_mu_anchor']
                    goal_emb_logvar_anchor = model_output['goal_emb_logvar_anchor']
                    
                    action_vae_reg_loss = vae_regularization_loss(goal_emb_mu_action, goal_emb_logvar_action)
                    action_vae_reg_loss = torch.nan_to_num(action_vae_reg_loss)
                    
                    anchor_vae_reg_loss = vae_regularization_loss(goal_emb_mu_anchor, goal_emb_logvar_anchor)
                    ...
                    action_mu = model_output["action_mu"]
                    action_logvar = model_output["action_logvar"]
                    anchor_mu = model_output["anchor_mu"]
                    anchor_logvar = model_output["anchor_logvar"]
                    
                    action_vae_reg_loss = vae_regularization_loss(action_mu, action_logvar)
                    action_vae_reg_loss = torch.nan_to_num(action_vae_reg_loss)
                    
                    anchor_vae_reg_loss = vae_regularization_loss(anchor_mu, anchor_logvar)
                    anchor_vae_reg_loss = torch.nan_to_num(anchor_vae_reg_loss)
                
                loss += self.vae_reg_loss_weight * (action_vae_reg_loss + anchor_vae_reg_loss)
                log_values[loss_prefix+'action_vae_reg_loss'] = self.vae_reg_loss_weight * action_vae_reg_loss
                log_values[loss_prefix+'anchor_vae_reg_loss'] = self.vae_reg_loss_weight * anchor_vae_reg_loss
                log_values[loss_prefix+'vae_reg_loss'] = self.vae_reg_loss_weight * (action_vae_reg_loss + anchor_vae_reg_loss)
            
            # Regularize the global latent z to be standard normal
            elif self.model.conditioning in ["hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                vae_reg_loss = vae_regularization_loss(model_output['goal_emb_mu'], model_output['goal_emb_logvar'])
                vae_reg_loss = torch.nan_to_num(vae_reg_loss)
        ...
        if self.spatial_distance_regularization_weight > 0:
            if self.model.conditioning in ["pos_delta_l2norm", "uniform_prior_pos_delta_l2norm", "distance_prior_pos_delta_l2norm"]:
                selected_anchor_point_idx = model_output['trans_sample_anchor']
                _, selected_anchor_point = self.model.get_dense_translation_point(points_anchor.permute(0, 2, 1)[:, :3], selected_anchor_point_idx, conditioning=self.model.conditioning)
                ...
                if self.spatial_distance_regularization_type == "pred":
                    selected_action_point_idx = model_output['trans_sample_action']
                    _, selected_action_point = self.model.get_dense_translation_point(pred_points_action.permute(0, 2, 1)[:, :3], selected_action_point_idx, conditioning=self.model.conditioning)
                # Compare the demo selected anchor point to the transformed selected anchor point, but don't backpropagate through TAXPose
                    ...
                    selected_action_point_idx = model_output['trans_sample_action']
                    _, selected_action_point = self.model.get_dense_translation_point(pred_points_action.permute(0, 2, 1)[:, :3].detach(), selected_action_point_idx, conditioning=self.model.conditioning)
                # Compare the demo selected anchor point to the demo selected action point
                    ...
                    selected_action_point_idx = model_output['trans_sample_action']
                    _, selected_action_point = self.model.get_dense_translation_point(points_action.permute(0, 2, 1)[:, :3], selected_action_point_idx, conditioning=self.model.conditioning)
                else:
                    raise ValueError("ERROR: Invalid spatial distance regularization type")
                    
                dist = (selected_action_point - selected_anchor_point).pow(2).sum(-1).sqrt().mean()
            
            # Do the same as above, but learn a distance vector offset for the selected anchor point
            elif self.model.conditioning in ["pos_delta_l2norm_dist_vec", "uniform_prior_pos_delta_l2norm_dist_vec", "distance_prior_pos_delta_l2norm_dist_vec"]:
                selected_anchor_point_idx = model_output['trans_sample_anchor']
                _, selected_anchor_point = self.model.get_dense_translation_point(points_anchor.permute(0, 2, 1)[:, :3], selected_anchor_point_idx, conditioning=self.model.conditioning) 
                
                dist_vec = model_output['dist_vec']
                selected_anchor_dist_vec = (dist_vec[:, :, N:] * selected_anchor_point_idx[:, None, :]).sum(axis=-1)
                ...
                dist = (selected_action_point - (selected_anchor_point + selected_anchor_dist_vec)).pow(2).sum(-1).sqrt().mean()
            
            # Do the same as above, but learn a distance scalar offset for the distance
            elif self.model.conditioning in ["pos_delta_l2norm_dist_scalar", "uniform_prior_pos_delta_l2norm_dist_scalar", "distance_prior_pos_delta_l2norm_dist_scalar"]:
                raise NotImplementedError("TODO: Implement this")
            
            else:
                raise ValueError("ERROR: Invalid conditioning type for spatial distance regularization")
            ...
            log_values[loss_prefix+'spatial_distance_regularization_loss'] = spatial_distance_loss
    ...
    def module_step(self, batch, batch_idx, log_prefix=''):
        ...
        if self.pzY_taxpose_infonce_loss_weight > 0:
            ...
            action_infonce_loss, action_infonce_log_values = compute_infonce_loss(
                ...
                points=model_outputs[0]['action_points_and_cond'].permute(0, 2, 1).detach(),
                center=action_center.permute(0, 2, 1).detach() if action_center is not None else None,
            ...
            anchor_infonce_loss, anchor_infonce_log_values = compute_infonce_loss(
                ...
                points=model_outputs[0]['anchor_points_and_cond'].permute(0, 2, 1).detach(),
                center=anchor_center.permute(0, 2, 1).detach() if anchor_center is not None else None,
                log_prefix=log_prefix+'anchor_',
            )
            
            loss += self.pzY_taxpose_infonce_loss_weight * (action_infonce_loss + anchor_infonce_loss)
            log_values = {**log_values, **action_infonce_log_values, **anchor_infonce_log_values}
        ...
        if self.pzY_taxpose_occ_infonce_loss_weight > 0:
            ...
            action_occ_infonce_loss, action_occ_infonce_log_values = compute_occlusion_infonce_loss(
                model=self.model.tax_pose.emb_nn_action,
                points=model_outputs[0]['action_points_and_cond'].permute(0, 2, 1).detach(),
                center=action_center.permute(0, 2, 1).detach() if action_center is not None else None,
                log_prefix=log_prefix+'action_',
            )
            anchor_occ_infonce_loss, anchor_occ_infonce_log_values = compute_occlusion_infonce_loss(
                model=self.model.tax_pose.emb_nn_anchor,
                points=model_outputs[0]['anchor_points_and_cond'].permute(0, 2, 1).detach(),
                center=anchor_center.permute(0, 2, 1).detach() if anchor_center is not None else None,
                log_prefix=log_prefix+'anchor_',
            ...
            loss += self.pzY_taxpose_occ_infonce_loss_weight * (action_occ_infonce_loss + anchor_occ_infonce_loss)
            log_values = {**log_values, **action_occ_infonce_log_values, **anchor_occ_infonce_log_values}
            ...
            with torch.no_grad():
                def get_inference_error(log_values, batch, loss_prefix):
                    ...
                    if self.model.conditioning not in ["uniform_prior_pos_delta_l2norm", "latent_z", "latent_z_1pred", "latent_z_1pred_10d", "latent_z_linear", "latent_z_linear_internalcond"]:
                        model_outputs = self.model(points_trans_action, points_trans_anchor, points_onetrans_action, points_onetrans_anchor, mode="forward", n_samples=1)
                    else:
                        model_outputs = self.model(points_trans_action, points_trans_anchor, points_onetrans_action, points_onetrans_anchor, mode="inference", n_samples=1)
                ...
                if self.get_errors_across_samples:
                    ...
                    for sampling_setting in sampling_settings:
                        if self.model.conditioning not in ["uniform_prior_pos_delta_l2norm", "latent_z", "latent_z_1pred", "latent_z_1pred_10d", "latent_z_linear", "latent_z_linear_internalcond"]:
                            model_outputs = self.model(points_trans_action, points_trans_anchor, points_onetrans_action, points_onetrans_anchor, mode="forward", sampling_method=sampling_setting[0], n_samples=sampling_setting[1])
                        else:
                            model_outputs = self.model(points_trans_action, points_trans_anchor, points_onetrans_action, points_onetrans_anchor, mode="inference", sampling_method=sampling_setting[0], n_samples=sampling_setting[1])
                    ...
                    for i in range(len(all_predictions)):
                        error_R_maxs, error_R_mins, error_R_means, error_t_maxs, error_t_mins, error_t_means = get_all_sample_errors(all_predictions[i], T0, T1, mode=self.error_mode_2rack, T_aug=T_aug)
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_mean'] = torch.mean(torch.Tensor(error_R_means))
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_mean'] = torch.mean(torch.Tensor(error_t_means))
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_max'] = torch.max(torch.Tensor(error_R_maxs))
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_max'] = torch.max(torch.Tensor(error_t_maxs))
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_min'] = torch.min(torch.Tensor(error_R_mins))
                        log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_min'] = torch.min(torch.Tensor(error_t_mins))
    ...
    def visualize_results(self, batch, batch_idx, log_prefix=''):
        ...
        if self.model.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond"]:
            if self.model.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                goal_emb = goal_emb[:, :1, :]
...
class EquivarianceTrainingModule_WithPZCondX(PointCloudTrainingModule):
    ...
    def get_transform(self, points_trans_action, points_trans_anchor, points_action=None, points_anchor=None, mode="forward", sampling_method="gumbel", n_samples=1):
        # mode is unused
        sample_latent = self.model_with_cond_x.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"] and self.hybrid_cond_pzX_sample_latent
    ...
    def compute_loss(self, model_output, batch, log_values={}, loss_prefix=''):
        ...
        if goal_emb is not None:
            ...
            if self.model_with_cond_x.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond", "hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                if self.goal_emb_cond_x_loss_type == "forward_kl":
                ...
                elif self.goal_emb_cond_x_loss_type.startswith("js_div_mod") or self.goal_emb_cond_x_loss_type.startswith("js_div_mod_eps0"):
                    eps = 1e-8 if self.goal_emb_cond_x_loss_type.startswith("js_div_mod") else 0
                    d_1 = float(self.goal_emb_cond_x_loss_type.split("_")[-2])
                    d_2 = float(self.goal_emb_cond_x_loss_type.split("_")[-1])
                    
                    action_kl = js_div_mod(q=F.log_softmax(goal_emb_cond_x[:, :, :N_action], dim=-1),
                                           p=F.log_softmax(goal_emb[:, :, :N_action], dim=-1),
                                           d_1=d_1, 
                                           d_2=d_2,
                                           reduction='batchmean',
                                           eps=eps)
                    anchor_kl = js_div_mod(q=F.log_softmax(goal_emb_cond_x[:, :, N_action:], dim=-1),
                                             p=F.log_softmax(goal_emb[:, :, N_action:], dim=-1),
                                             d_1=d_1, 
                                             d_2=d_2,
                                             reduction='batchmean',
                    ...
                    action_kl = wasserstein_distance(F.softmax(goal_emb_cond_x[:, :, :N_action], dim=-1),
                                                     F.softmax(goal_emb[:, :, :N_action], dim=-1), 
                                                     p_exp=p_exp)
                    anchor_kl = wasserstein_distance(F.softmax(goal_emb_cond_x[:, :, N_action:], dim=-1),
                                                     F.softmax(goal_emb[:, :, N_action:], dim=-1), 
                                                     p_exp=p_exp)
            ...
            elif self.model_with_cond_x.conditioning in ["latent_z_linear", "latent_z_linear_internalcond"]:
                goal_emb_mu = model_output['pzY_goal_emb_mu']
                goal_emb_logvar = model_output['pzY_goal_emb_logvar']
                
                goal_emb_cond_x_mu = model_output['goal_emb_mu']
                goal_emb_cond_x_logvar = model_output['goal_emb_logvar']
                ...
                def latent_kl(mu1, logvar1, mu2, logvar2, reduction='batchmean'):
                    ...
                    kl = 0.5 * (logvar2 - logvar1) + (torch.exp(logvar1) + (mu1 - mu2)**2) / (2 * torch.exp(logvar2) + 1e-8) - 0.5
                ...
                if self.goal_emb_cond_x_loss_type == "forward_kl":
                    latent_goal_kl = latent_kl(mu1=goal_emb_mu, logvar1=goal_emb_logvar,
                                               mu2=goal_emb_cond_x_mu, logvar2=goal_emb_cond_x_logvar,
                                               reduction='batchmean')
                    
                    action_kl = latent_goal_kl
                    anchor_kl = latent_goal_kl
                elif self.goal_emb_cond_x_loss_type == "reverse_kl":
                    latent_goal_kl = latent_kl(mu1=goal_emb_cond_x_mu, logvar1=goal_emb_cond_x_logvar,
                                               mu2=goal_emb_mu, logvar2=goal_emb_logvar,
                                               reduction='batchmean')
                    ...
                    def compute_js_loss(source_mu, source_log_var, target_mu, target_log_var):
                        ...
                        def get_prob(mu, log_var):
                            dist = torch.distributions.Normal(mu, torch.exp(0.5 * log_var))
                            val = dist.sample()
                    ...
                    for n_sample in range(n_samples):
                        latent_goal_kl += compute_js_loss(source_mu=goal_emb_mu, source_log_var=goal_emb_logvar,
                                                          target_mu=goal_emb_cond_x_mu, target_log_var=goal_emb_cond_x_logvar)
                    latent_goal_kl /= n_samples
                    
                    action_kl = latent_goal_kl
                    anchor_kl = latent_goal_kl
                    
            elif self.model_with_cond_x.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                ...
                if self.hybrid_cond_pzX_regularize_type in ["all", "selected"]:
                    goal_emb_mu_action = model_output['pzY_goal_emb_mu_action']
                    goal_emb_mu_anchor = model_output['pzY_goal_emb_mu_anchor']
                    goal_emb_logvar_action = model_output['pzY_goal_emb_logvar_action']
                    goal_emb_logvar_anchor = model_output['pzY_goal_emb_logvar_anchor']
                    goal_emb_selected_action_mu = model_output['pzY_action_mu']
                    goal_emb_selected_anchor_mu = model_output['pzY_anchor_mu']
                    goal_emb_selected_action_logvar = model_output['pzY_action_logvar']
                    goal_emb_selected_anchor_logvar = model_output['pzY_anchor_logvar']
                    
                    goal_emb_cond_x_mu_action = model_output['goal_emb_mu_action']
                    goal_emb_cond_x_mu_anchor = model_output['goal_emb_mu_anchor']
                    goal_emb_cond_x_logvar_action = model_output['goal_emb_logvar_action']
                    goal_emb_cond_x_logvar_anchor = model_output['goal_emb_logvar_anchor']
                    goal_emb_cond_x_selected_action_mu = model_output['action_mu']
                    goal_emb_cond_x_selected_anchor_mu = model_output['anchor_mu']
                    goal_emb_cond_x_selected_action_logvar = model_output['action_logvar']
                    goal_emb_cond_x_selected_anchor_logvar = model_output['anchor_logvar']
                ...
                if self.goal_emb_cond_x_loss_type == "forward_kl":
                    ...
                    discrete_action_kl = F.kl_div(F.log_softmax(goal_emb_cond_x_discrete_action, dim=-1),
                                         F.log_softmax(goal_emb_discrete_action, dim=-1), 
                                         log_target=True,
                                         reduction='batchmean')
                    discrete_anchor_kl = F.kl_div(F.log_softmax(goal_emb_cond_x_discrete_anchor, dim=-1),
                    ...
                    if self.hybrid_cond_pzX_regularize_type == "all":
                        ...
                        latent_action_kl = latent_kl(mu1=goal_emb_mu_action, logvar1=goal_emb_logvar_action, 
                                                     mu2=goal_emb_cond_x_mu_action, logvar2=goal_emb_cond_x_logvar_action,
                                                     reduction='batchmean')
                        latent_anchor_kl = latent_kl(mu1=goal_emb_mu_anchor, logvar1=goal_emb_logvar_anchor,
                                                     mu2=goal_emb_cond_x_mu_anchor, logvar2=goal_emb_cond_x_logvar_anchor,
                                                     reduction='batchmean')
                        ...
                        latent_action_kl = latent_kl(mu1=goal_emb_selected_action_mu, logvar1=goal_emb_selected_action_logvar,
                                                     mu2=goal_emb_cond_x_selected_action_mu, logvar2=goal_emb_cond_x_selected_action_logvar,
                                                     reduction='batchmean')
                        latent_anchor_kl = latent_kl(mu1=goal_emb_selected_anchor_mu, logvar1=goal_emb_selected_anchor_logvar,
                                                     mu2=goal_emb_cond_x_selected_anchor_mu, logvar2=goal_emb_cond_x_selected_anchor_logvar,
                                                     reduction='batchmean')
                        ...
                        latent_global_kl = latent_kl(mu1=goal_emb_mu, logvar1=goal_emb_logvar,
                                                     mu2=goal_emb_cond_x_mu, logvar2=goal_emb_cond_x_logvar,
                                                     reduction='batchmean')
                        ...
                        raise ValueError(f'hybrid_cond_pzX_regularize_type={self.hybrid_cond_pzX_regularize_type} not supported')
                    
                elif self.goal_emb_cond_x_loss_type == "reverse_kl":
                    discrete_action_kl = F.kl_div(F.log_softmax(goal_emb_discrete_action, dim=-1),
                                         F.log_softmax(goal_emb_cond_x_discrete_action, dim=-1), 
                                         log_target=True,
                                         reduction='batchmean')
                    discrete_anchor_kl = F.kl_div(F.log_softmax(goal_emb_discrete_anchor, dim=-1),
                    ...
                    if self.hybrid_cond_pzX_regularize_type == "all":
                        latent_action_kl = latent_kl(mu1=goal_emb_cond_x_mu_action, logvar1=goal_emb_cond_x_logvar_action,
                                                     mu2=goal_emb_mu_action, logvar2=goal_emb_logvar_action,
                                                     reduction='batchmean')
                        latent_anchor_kl = latent_kl(mu1=goal_emb_cond_x_mu_anchor, logvar1=goal_emb_cond_x_logvar_anchor,
                                                     mu2=goal_emb_mu_anchor, logvar2=goal_emb_logvar_anchor,
                                                     reduction='batchmean')
                        
                        action_kl = discrete_action_kl + latent_action_kl
                        anchor_kl = discrete_anchor_kl + latent_anchor_kl
                    elif self.hybrid_cond_pzX_regularize_type == "selected":
                        latent_action_kl = latent_kl(mu1=goal_emb_cond_x_selected_action_mu, logvar1=goal_emb_cond_x_selected_action_logvar,
                                                     mu2=goal_emb_selected_action_mu, logvar2=goal_emb_selected_action_logvar,
                                                     reduction='batchmean')
                        latent_anchor_kl = latent_kl(mu1=goal_emb_cond_x_selected_anchor_mu, logvar1=goal_emb_cond_x_selected_anchor_logvar,
                                                     mu2=goal_emb_selected_anchor_mu, logvar2=goal_emb_selected_anchor_logvar,
                                                     reduction='batchmean')
                        ...
                        latent_global_kl = latent_kl(mu1=goal_emb_cond_x_mu, logvar1=goal_emb_cond_x_logvar,
                                                     mu2=goal_emb_mu, logvar2=goal_emb_logvar,
                                                     reduction='batchmean')
                    ...
                    eps = 1e-8 if self.goal_emb_cond_x_loss_type == "js_div" else 0
                    discrete_action_kl = js_div(q=F.log_softmax(goal_emb_cond_x_discrete_action, dim=-1),
                                       p=F.log_softmax(goal_emb_discrete_action, dim=-1),
                                       reduction='batchmean',
                                       eps=eps)
                    discrete_anchor_kl = js_div(q=F.log_softmax(goal_emb_cond_x_discrete_anchor, dim=-1),
                    ...
                    raise ValueError(f'goal_emb_cond_x_loss_type={self.goal_emb_cond_x_loss_type} not supported for {self.model_with_cond_x.conditioning}')
            ...
            if self.model_with_cond_x.conditioning in ["latent_z_linear", "latent_z_linear_internalcond"]:
    ...
    def module_step(self, batch, batch_idx, log_prefix=''):
        ...
        if self.joint_infonce_loss_weight > 0:
            ...
            if self.model_with_cond_x.encoder_type in ['1_dgcnn']:
                raise NotImplementedError("joint_pretraining_loss not implemented for 1_dgcnn")
            # Two encoders
            else:
                ...
                loss += self.joint_infonce_loss_weight * (action_infonce_loss + anchor_infonce_loss)
                log_values = {**log_values, **action_infonce_log_values, **anchor_infonce_log_values}
        ...
        if self.get_errors_across_samples:
            with torch.no_grad():
                ...
                all_model_outputs = []
                for sampling_setting in sampling_settings:
                    error_model_outputs = self.model_with_cond_x(points_trans_action, 
                                                           points_trans_anchor, 
                                                           points_onetrans_action, 
                                                           points_onetrans_anchor,
                ...
                for i in range(len(all_model_outputs)):
                    ...
                    for j in range(len(all_model_outputs[i])):
                        ...
                        if "sampled_ixs_action" in error_model_outputs[i][j]:
                            ixs_action = error_model_outputs[i][j]["sampled_ixs_action"].unsqueeze(-1)
                            sampled_points_trans_action = torch.take_along_dim(
                                points_trans_action, ixs_action, dim=1
                            )
                        else:
                        ...
                        if "sampled_ixs_anchor" in error_model_outputs[i][j]:
                            ixs_anchor = error_model_outputs[i][j]["sampled_ixs_anchor"].unsqueeze(-1)
                            sampled_points_trans_anchor = torch.take_along_dim(
                                points_trans_anchor, ixs_anchor, dim=1
                            )
                        else:
                ...
                for i in range(len(all_predictions)):
                    error_R_maxs, error_R_mins, error_R_means, error_t_maxs, error_t_mins, error_t_means = get_all_sample_errors(all_predictions[i], T0, T1, mode=self.training_module_no_cond_x.error_mode_2rack, T_aug=T_aug)
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_mean'] = torch.mean(torch.Tensor(error_R_means))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_mean'] = torch.mean(torch.Tensor(error_t_means))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_max'] = torch.max(torch.Tensor(error_R_maxs))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_max'] = torch.max(torch.Tensor(error_t_maxs))
