...
class EquivarianceTrainingModule(PointCloudTrainingModule):
    ...
    def compute_loss(self, model_outputs, batch, log_values={}, loss_prefix='', heads_list=None):
        ...
        if heads is not None or self.model.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
            ...
            # Regularize the global latent z to be standard normal
                ...
                vae_reg_loss = vae_regularization_loss(model_outputs[0]['goal_emb_mu'], model_outputs[0]['goal_emb_logvar'])
                vae_reg_loss = torch.nan_to_num(vae_reg_loss)
        ...
        if self.spatial_distance_regularization_weight > 0:
            if self.model.conditioning in ["pos_delta_l2norm", "uniform_prior_pos_delta_l2norm", "distance_prior_pos_delta_l2norm", "hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_global"]:
                ...
                _, selected_anchor_point = self.model.get_dense_translation_point(
                    ...
                    conditioning=self.model.conditioning if re.match(r"^hybrid_.*$", self.model.conditioning) is None else "pos_delta_l2norm"
                )
    ...
    def module_step(self, batch, batch_idx, log_prefix=''):
        ...
        loss, log_values = self.compute_loss(
            model_outputs, batch, log_values=log_values, loss_prefix=log_prefix, heads_list=heads)
        ...
        for model_output in model_outputs:
            if self.pzY_taxpose_infonce_loss_weight > 0:
                ...
                action_infonce_loss, action_infonce_log_values = compute_infonce_loss(
                    ...
                    points=model_output['action_points_and_cond'].permute(0, 2, 1).detach(),
                ...
                anchor_infonce_loss, anchor_infonce_log_values = compute_infonce_loss(
                    ...
                    points=model_output['anchor_points_and_cond'].permute(0, 2, 1).detach(),
                    center=anchor_center.permute(0, 2, 1).detach() if anchor_center is not None else None,
                    log_prefix=log_prefix+'anchor_',
                )
                
                aux_losses += self.pzY_taxpose_infonce_loss_weight * (action_infonce_loss + anchor_infonce_loss)
            ...
            if self.pzY_taxpose_occ_infonce_loss_weight > 0:
                ...
                aux_losses += self.pzY_taxpose_occ_infonce_loss_weight * (action_occ_infonce_loss + anchor_occ_infonce_loss)
            ...
            with torch.no_grad():
                ...
                if self.get_errors_across_samples:
                    ...
                    for i in range(len(all_predictions)):
                        error_R_maxs, error_R_mins, error_R_means, error_t_maxs, error_t_mins, error_t_means = get_all_sample_errors(all_predictions[i], T0, T1, mode=self.error_mode_2rack, T_aug_list=T_aug_list)
        ...
        if self.model.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond"]:
            ...
            if self.plot_encoder_distribution:
                ...
                if 'points_action_aug_trans' in batch:
                    ...
                    assert points_action_aug_trans.shape[1] == points_onetrans_anchor.shape[1], \
...
class EquivarianceTrainingModule_WithPZCondX(PointCloudTrainingModule):
    ...
    def compute_loss(self, model_outputs, batch, log_values={}, loss_prefix=''):
        ...
        loss, log_values = self.training_module_no_cond_x.compute_loss(model_outputs, batch, log_values, loss_prefix)
        ...
        if goal_emb is not None:
            ...
            if self.model_with_cond_x.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond", "hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                ...
                if self.hybrid_cond_pzX_regularize_type in ["all", "selected"]:
                    ...
                    goal_emb_logvar_action = model_outputs[0]['pzY_goal_emb_logvar_action']
                    goal_emb_logvar_anchor = model_outputs[0]['pzY_goal_emb_logvar_anchor']
                    goal_emb_selected_action_mu = model_outputs[0]['pzY_action_mu']
                    goal_emb_selected_anchor_mu = model_outputs[0]['pzY_anchor_mu']
                    goal_emb_selected_action_logvar = model_outputs[0]['pzY_action_logvar']
                    goal_emb_selected_anchor_logvar = model_outputs[0]['pzY_anchor_logvar']
                    
                    goal_emb_cond_x_mu_action = model_outputs[0]['goal_emb_mu_action']
                    goal_emb_cond_x_mu_anchor = model_outputs[0]['goal_emb_mu_anchor']
                    goal_emb_cond_x_logvar_action = model_outputs[0]['goal_emb_logvar_action']
                    goal_emb_cond_x_logvar_anchor = model_outputs[0]['goal_emb_logvar_anchor']
                    goal_emb_cond_x_selected_action_mu = model_outputs[0]['action_mu']
                    goal_emb_cond_x_selected_anchor_mu = model_outputs[0]['anchor_mu']
                    goal_emb_cond_x_selected_action_logvar = model_outputs[0]['action_logvar']
                    goal_emb_cond_x_selected_anchor_logvar = model_outputs[0]['anchor_logvar']
    ...
    def module_step(self, batch, batch_idx, log_prefix=''):
        ...
        if self.get_errors_across_samples:
            with torch.no_grad():
                ...
                for i in range(len(all_predictions)):
                    error_R_maxs, error_R_mins, error_R_means, error_t_maxs, error_t_mins, error_t_means = get_all_sample_errors(all_predictions[i], T0, T1, mode=self.training_module_no_cond_x.error_mode_2rack, T_aug_list=T_aug_list)
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_mean'] = torch.mean(torch.Tensor(error_R_means))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_mean'] = torch.mean(torch.Tensor(error_t_means))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_R_max'] = torch.max(torch.Tensor(error_R_maxs))
                    log_values[log_prefix+f'{sampling_settings[i][0]}-{sampling_settings[i][1]}_error_t_max'] = torch.max(torch.Tensor(error_t_maxs))
