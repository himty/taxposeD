...
from equivariant_pose_graph.utils.se3 import dualflow2pose, flow2pose, get_translation, get_degree_angle, pure_translation_se3
from equivariant_pose_graph.utils.display_headless import scatter3d, quiver3d
from equivariant_pose_graph.utils.color_utils import get_color, create_axis
...
class EquivarianceTestingModule(PointCloudTrainingModule):
    ...
    def get_transform(self, points_trans_action, points_trans_anchor):
        for i in range(self.loop):
            ...
            if i == 0:
                ...
                pred_T_action = pred_T_action.compose(T_trans.inverse().compose(
                    ans_dict["pred_T_action"]).compose(T_trans))
                ans_dict["pred_T_action"] = pred_T_action
            pred_points_action = ans_dict["pred_points_action"]
            points_trans_action, points_trans_anchor, points_action_mean = self.action_centered(
    ...
    def visualize_results(self, batch, batch_idx):
        ...
        for i in range(self.loop):
            ...
            ans_dict = self.predict(x_action=x_action, x_anchor=x_anchor,
                                    points_trans_action=pred_points_action, points_trans_anchor=points_trans_anchor)

            demo_points_apply_action_transform = get_color(
                tensor_list=[pred_points_action[0],
