...
class EquivarianceTrainingModule(PointCloudTrainingModule):
    ...
    def cal_loss(self, x_action, x_anchor, batch, log_values={}, loss_prefix=''):
        ...
        pred_T_action = flow2pose(
            points_trans_action, pred_flow_action, pred_w_action, return_transform3d=True, normalization_scehme=self.weight_normalize)
        induced_flow_action = (pred_T_action.transform_points(
            points_trans_action) - points_trans_action).detach()
        pred_points_action = pred_T_action.compose(
            T1.inverse()).transform_points(points_trans_action)
        ...
        pred_T_anchor = flow2pose(
            points_trans_anchor, pred_flow_anchor, pred_w_anchor, return_transform3d=True, normalization_scehme=self.weight_normalize)
        induced_flow_anchor = (pred_T_anchor.transform_points(
            points_trans_anchor) - points_trans_anchor).detach()
        pred_points_anchor = pred_T_anchor.compose(
            T0.inverse()).transform_points(points_trans_anchor)
        ...
        if self.point_loss_type == 0:
            ...
            # chamfer loss
            point_loss_action = self.chamfer_weight * chamfer_distance(
                pred_points_action,
                points_action,
            )[0].item()
            point_loss_anchor = self.chamfer_weight * chamfer_distance(
                pred_points_anchor,
                points_anchor,
            )[0].item()
        elif self.point_loss_type == 2:
            # chamfer loss + mse loss
            point_loss_action_mse = mse_criterion(
                pred_points_action,
                points_action)
            point_loss_anchor_mse = mse_criterion(
                pred_points_anchor,
                points_anchor)

            point_loss_action_chamfer = self.chamfer_weight * chamfer_distance(
                pred_points_action,
                points_action)[0].item()
            point_loss_anchor_chamfer = self.chamfer_weight * chamfer_distance(
                pred_points_anchor,
                points_anchor)[0].item()
        ...
        log_values[loss_prefix +
                   'smoothness_loss_action'] = smoothness_loss_action
        log_values[loss_prefix +
                   'smoothness_loss_anchor'] = smoothness_loss_anchor
        ...
        if self.point_loss_type == 2:
            log_values[loss_prefix +
                       'point_loss_action_mse'] = point_loss_action_mse
            log_values[loss_prefix +
                       'point_loss_anchor_mse'] = point_loss_anchor_mse
            log_values[loss_prefix +
                       'point_loss_action_chamfer'] = point_loss_action_chamfer
            log_values[loss_prefix +
                       'point_loss_anchor_chamfer'] = point_loss_anchor_chamfer
    ...
    def forward(self, points_actions, points_anchor):
        ...
        pred_T_action = flow2pose(
            points_actions, pred_flow_action, pred_w_action, return_transform3d=True, normalization_scehme=self.weight_normalize)
        pred_T_anchor = flow2pose(
            points_anchor, pred_flow_anchor, pred_w_anchor, return_transform3d=True, normalization_scehme=self.weight_normalize)
    ...
    def module_step(self, batch, batch_idx):
        ...
        if self.return_flow_component:
            loss_residual, log_values = self.cal_loss(
                residual_flow_action, residual_flow_anchor, batch, log_values=log_values, loss_prefix='residual_')
            loss_corr, log_values = self.cal_loss(
                corr_flow_action, corr_flow_anchor, batch, log_values=log_values, loss_prefix='corres_')
    ...
    def visualize_results(self, batch, batch_idx):
        ...
        pred_points_anchor = pred_T_anchor.compose(
            T0.inverse()).transform_points(points_trans_anchor)
        action_transformed = pred_T_action.transform_points(
            points_trans_action)
        anchor_transformed = pred_T_anchor.transform_points(
            points_trans_anchor)

        points = torch.cat([points_action, points_anchor], dim=1)
        points_trans = torch.cat(
            [points_trans_action, points_trans_anchor], dim=1)
        ...
        res_images['all_input_points'] = scatter3d(
            xs=[points_target_disp, points_trans_disp],
            cs=['b', 'r']
        )
        res_images['input_points_action'] = quiver3d(
            xs=[points_target_disp_action[::index_interval],
        ...
        res_images['input_points_anchor'] = quiver3d(
            xs=[points_target_disp_anchor[::index_interval],
        ...
        if(self.display_action):
            ...
            res_images['action_points'] = scatter3d(
                xs=[points_target_disp, points_trans_anchored_disp,
