...
class MultiheadRelativeAttentionWrapper(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, add_bias_kv=False, add_zero_attn=False,
                 kdim=None, vdim=None, slot_competition=False, return_kv=False, gate_attn=False):
        super().__init__()
        self.attn = MultiheadRelativeAttention(embed_dim, num_heads, dropout, bias, add_bias_kv, add_zero_attn, kdim,
                                               vdim, slot_competition, return_kv, gate_attn)

    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None, k_mem=None,
                v_mem=None, mem_mask=None, rotary_pe=None):
        query_in = query.permute(1, 0, 2) # (batch_size, seq_len, embed_dim) -> (seq_len, batch_size, embed_dim)
        key_in = key.permute(1, 0, 2)
        value_in = value.permute(1, 0, 2)
        
        output = self.attn(query_in, key_in, value_in, key_padding_mask, need_weights, attn_mask, k_mem, v_mem, mem_mask, rotary_pe)
        
        attn_output = output[0]
        attn_output = attn_output.permute(1, 0, 2) # (seq_len, batch_size, embed_dim) -> (batch_size, seq_len, embed_dim)
...
class RelativeAttentionLayer(nn.Module):
    def __init__(self, embedding_dim: int = 516, num_heads: int = 4, dropout: float = 0.0):
        super().__init__()
        self.multihead_attn = MultiheadRelativeAttention(embedding_dim, num_heads, dropout=dropout)
...
class FeedforwardLayer(nn.Module):
    def __init__(self, embedding_dim: int = 516, hidden_dim: int = 516, dropout: float = 0.0):
        super().__init__()
...
class RelativeSelfAttentionModule(nn.Module):
    def __init__(self, embedding_dim: int = 516, num_attn_heads: int = 4, num_layers: int = 2):
        super().__init__()

        self.attn_layers = nn.ModuleList()
        self.ffw_layers = nn.ModuleList()
        for _ in range(num_layers):
            self.attn_layers.append(RelativeAttentionLayer(embedding_dim, num_attn_heads))
