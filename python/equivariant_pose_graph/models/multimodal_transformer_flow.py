...
from equivariant_pose_graph.utils.sample_utils import sample_closest_pairs, sample_random_pair
...
class DGCNNHybrid(nn.Module):
    ...
    def __init__(self, emb_dims=512, input_dims=3, num_heads=1, conditioning_size=0, last_relu=True, dropout=0.5, output_channels=40):
...
class Multimodal_ResidualFlow_DiffEmbTransformer(nn.Module):
    ...
    TP_INPUT_DIMS = {
        ...
        'hybrid_pos_delta_l2norm': 3 + 1, # Add cfg.latent_z_linear_size to this in main script because its dynamic, hacky
        'hybrid_pos_delta_l2norm_internalcond': 3 + 1, # Add cfg.latent_z_linear_size to this in main script because its dynamic, hacky
        'hybrid_pos_delta_l2norm_global': 3 + 1, # Add cfg.latent_z_linear_size to this in main script because its dynamic, hacky
        'hybrid_pos_delta_l2norm_global_internalcond': 3 + 1, # Add cfg.latent_z_linear_size to this in main script because its dynamic, hacky
    ...
    def __init__(self, residualflow_diffembtransformer, gumbel_temp=0.5, freeze_residual_flow=False, center_feature=False, freeze_z_embnn=False,
                 ...
                 taxpose_centering="mean", use_action_z=True, pzY_encoder_type="dgcnn", pzY_transformer="none",
                 pzY_transformer_embnn_dims=512, pzY_transformer_emb_dims=512, pzY_input_dims=3, pzY_dropout_goal_emb=0.0,
                 pzY_embedding_routine="joint", pzY_embedding_option=0, hybrid_cond_logvar_limit=0.0, latent_z_cond_logvar_limit=0.0,
        ...
        if self.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
        ...
        if self.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
            # Single encoder
            if self.pzY_encoder_type == "dgcnn":
                print(f'--- P(z|Y) Using 1 DGCNN ---')
                self.emb_nn_objs_at_goal = DGCNN(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, last_relu=False)
                ...
                args = VNArgs()
                self.emb_nn_objs_at_goal = VN_DGCNN(args, num_part=self.emb_dims, gc=False)
            elif self.pzY_encoder_type == "pn++_msg":
                print(f'--- P(z|Y) Using 1 PN++ MSG ---')
                self.emb_nn_objs_at_goal = PointNet2MSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            elif self.pzY_encoder_type == "pn++_ssg":
                print(f'--- P(z|Y) Using 1 PN++ SSG ---')
                self.emb_nn_objs_at_goal = PointNet2SSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            ...
            # Two encoders
                ...
                self.emb_nn_action_at_goal = DGCNN(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, last_relu=False)
                self.emb_nn_anchor_at_goal = DGCNN(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, last_relu=False)
            elif self.pzY_encoder_type == "2_pn++_msg":
                print(f'--- P(z|Y) Using 2 PN++ MSG ---')
                self.emb_nn_action_at_goal = PointNet2MSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
                self.emb_nn_anchor_at_goal = PointNet2MSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            elif self.pzY_encoder_type == "2_pn++_ssg":
                print(f'--- P(z|Y) Using 2 PN++ SSG ---')
                self.emb_nn_action_at_goal = PointNet2SSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
                self.emb_nn_anchor_at_goal = PointNet2SSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            elif self.pzY_encoder_type == "2_pn++":
                print(f'--- P(z|Y) Using 2 PyG PN++ ---')
                args = PN2DenseParams()
                self.emb_nn_action_at_goal = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.emb_dims, p=args)
                self.emb_nn_anchor_at_goal = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.emb_dims, p=args)
            else:
                raise ValueError(f"pzY_encoder_type {self.pzY_encoder_type} not implemented for conditioning {self.conditioning}")
        elif self.conditioning in ["hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
            if self.pzY_encoder_type == 'dgcnn':
                print(f'--- P(z|Y) Using 1 DGCNN Hybrid ---')
                self.emb_nn_objs_at_goal = DGCNNHybrid(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, last_relu=False, output_channels=self.latent_z_linear_size)
            elif self.pzY_encoder_type == 'pn++':
                print(f'--- P(z|Y) Using 1 PyG PN++ Hybrid ---')
                args = PN2DenseParams()
                self.emb_nn_objs_at_goal = PN2HybridWrapper(in_channels=self.input_dims - 3, out_channels=self.latent_z_linear_size, num_heads=self.num_emb_heads, emb_dims=self.emb_dims, p=args)
        else:
            if self.pzY_encoder_type == "dgcnn":
                print(f'--- P(z|Y) Using 1 DGCNN Classification ---')
                self.emb_nn_objs_at_goal = DGCNNClassification(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, dropout=0.5, output_channels=self.latent_z_linear_size)
            elif self.pzY_encoder_type == "pn++":
        ...
        if self.pzY_transformer != "none":
            if self.pzY_transformer == "cross_object":
                ...
                if self.pzY_encoder_type == "2_dgcnn":
                    ...
                    self.emb_nn_action_at_goal = DGCNN(input_dims=self.input_dims, emb_dims=self.pzY_transformer_embnn_dims, num_heads=self.num_emb_heads, last_relu=False)
                    self.emb_nn_anchor_at_goal = DGCNN(input_dims=self.input_dims, emb_dims=self.pzY_transformer_embnn_dims, num_heads=self.num_emb_heads, last_relu=False)
                elif self.pzY_encoder_type == "2_pn++_msg":
                    print(f'------ With 2 PN++ MSG Encoders ------')
                    self.emb_nn_action_at_goal = PointNet2MSG(num_classes=self.pzY_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                    self.emb_nn_anchor_at_goal = PointNet2MSG(num_classes=self.pzY_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                elif self.pzY_encoder_type == "2_pn++_ssg":
                    print(f'------ With 2 PN++ SSG Encoders ------')
                    self.emb_nn_action_at_goal = PointNet2SSG(num_classes=self.pzY_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                    self.emb_nn_anchor_at_goal = PointNet2SSG(num_classes=self.pzY_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                elif self.pzY_encoder_type == "2_pn++":
                    print(f'------ With 2 PyG PN++ Encoders ------')
                    args = PN2DenseParams()
                    self.emb_nn_action_at_goal = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.pzY_transformer_embnn_dims, p=args)
                    self.emb_nn_anchor_at_goal = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.pzY_transformer_embnn_dims, p=args)
                else:
                    raise ValueError(f"pzY_transformer {self.pzY_transformer} not implemented for encoder_type {self.pzY_encoder_type}")
                
                self.action_transformer = Transformer(emb_dims=self.pzY_transformer_emb_dims, return_attn=True, bidirectional=False)
                self.anchor_transformer = Transformer(emb_dims=self.pzY_transformer_emb_dims, return_attn=True, bidirectional=False)
        ...
        if self.embedding_routine in ["joint2global"]:
            assert self.conditioning in ["hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"], "joint2global only supported for hybrid global conditioning"
            ...
            if self.cond_embnn_type == "pn++":
                args = PN2EncoderParams()
                self.cond_embnn = PN2EncoderWrapper(in_channels=self.input_dims - 3, out_channels=self.latent_z_linear_size, num_heads=self.num_emb_heads, emb_dims=self.emb_dims, p=args)
...
#        dense = torch.exp(-dense.abs() / 0.1)
    ...
    def sample_dense_embedding(self, goal_emb, sampling_method='gumbel', n_samples=1):
        ...
        if sampling_method == 'gumbel' and n_samples > 1:
            khot, onehot_list = gumbel_softmax_topk(goal_emb, k=n_samples, tau=self.gumbel_temp, hard=True, dim=-1)
            samples = onehot_list
        else:
    ...
    def add_single_conditioning(self, goal_emb, points, conditioning, cond_type='action', sampling_method='gumbel', n_samples=1, z_samples=None):
        ...
        if conditioning in ['pos_delta_vec', 'pos_loc3d', 'pos_onehot', 'pos_delta_l2norm', 'pos_exp_delta_l2norm', 'uniform_prior_pos_delta_l2norm', 'distance_prior_pos_delta_l2norm', 'pos_delta_l2norm_dist_vec', 'uniform_prior_pos_delta_l2norm_dist_vec', 'distance_prior_pos_delta_l2norm_dist_vec']:
        ...
        elif conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
            assert self.latent_z_linear_size % 2 == 0, "latent_z_linear_size must be even for hybrid conditioning"
            ...
            goal_emb_dense = (goal_emb_dense + self.add_smooth_factor) / self.division_smooth_factor
            ...
            for translation_sample in translation_samples:
                ...
                if conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                    ...
                    if self.hybrid_cond_logvar_limit > 0: # Hacky way to prevent large logvar causing NaNs
                        goal_emb_logvar = self.hybrid_cond_logvar_limit * torch.tanh(goal_emb_logvar)
                    
                    # Get the latent z corresponding to the selected action/anchor points
                    selected_mu = (goal_emb_mu*translation_sample[:,None,:]).sum(axis=2, keepdim=True)
                    selected_logvar = (goal_emb_logvar*translation_sample[:,None,:]).sum(axis=2, keepdim=True)
                ...
                # Also get a global latent z
                elif conditioning in ["hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                    ...
                    raise ValueError(f"Hybrid conditioning {conditioning} does not exist.")
                ...
                if conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_global"]:
                    points_and_cond = torch.cat([
                        points_and_cond,
                        torch.tile(selected_latent, (1, 1, selected_latent.shape[-1]))
                    ], axis=1)
    ...
    def add_conditioning(self, goal_emb, action_points, anchor_points, conditioning, sampling_method='gumbel', n_samples=1, z_samples=None):
        ...
        if conditioning in ['pos_delta_vec', 'pos_loc3d', 'pos_onehot', 'pos_delta_l2norm', 'pos_exp_delta_l2norm', 'uniform_prior_pos_delta_l2norm', 'distance_prior_pos_delta_l2norm', 'pos_delta_l2norm_dist_vec', 'uniform_prior_pos_delta_l2norm_dist_vec', 'distance_prior_pos_delta_l2norm_dist_vec']:
            ...
            translation_samples_action = Multimodal_ResidualFlow_DiffEmbTransformer.sample_dense_embedding(
                self, 
            ...
            for translation_sample_action, translation_sample_anchor in zip(translation_samples_action, translation_samples_anchor):
                # This is the only line that's different among the 3 different conditioning schemes in this category
                dense_trans_pt_action, ref_action = Multimodal_ResidualFlow_DiffEmbTransformer.get_dense_translation_point(
                    None, 
                    action_points, 
                    translation_sample_action, 
                    conditioning=conditioning
                )
                dense_trans_pt_anchor, ref_anchor = Multimodal_ResidualFlow_DiffEmbTransformer.get_dense_translation_point(
                    None, 
                    anchor_points, 
            ...
            for i in range(n_samples):
                ...
                if conditioning == "latent_z_linear":
                    action_points_and_cond = torch.cat([action_points] + [torch.tile(goal_emb_sample, (1, 1, action_points.shape[-1]))], axis=1)
                    anchor_points_and_cond = torch.cat([anchor_points] + [torch.tile(goal_emb_sample, (1, 1, anchor_points.shape[-1]))], axis=1)
            ...
            if conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                ...
                if self.hybrid_cond_logvar_limit > 0: # Hacky way to prevent large logvar causing NaNs
                    goal_emb_logvar_action = self.hybrid_cond_logvar_limit * torch.tanh(goal_emb_logvar_action)
                    goal_emb_logvar_anchor = self.hybrid_cond_logvar_limit * torch.tanh(goal_emb_logvar_anchor)
            ...
            for translation_sample_action, translation_sample_anchor in zip(translation_samples_action, translation_samples_anchor):
                ...
                if self.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                    # Get the latent z corresponding to the selected action/anchor points
                    action_mu = (goal_emb_mu_action*translation_sample_action[:,None,:]).sum(axis=2, keepdim=True)
                    action_logvar = (goal_emb_logvar_action*translation_sample_action[:,None,:]).sum(axis=2, keepdim=True)
                    action_latent = reparametrize(action_mu, action_logvar)
                    
                    anchor_mu = (goal_emb_mu_anchor*translation_sample_anchor[:,None,:]).sum(axis=2, keepdim=True)
                    anchor_logvar = (goal_emb_logvar_anchor*translation_sample_anchor[:,None,:]).sum(axis=2, keepdim=True)
    ...
    def forward(self, *input, mode="forward", sampling_method="gumbel", n_samples=1, z_samples=None):
        ...
        if mode == "forward":
            ...
            if self.embedding_routine == "joint":
                ...
                with torch.set_grad_enabled(not self.freeze_z_embnn):
                    if self.pzY_encoder_type in ["2_dgcnn", "2_pn++_msg", "2_pn++_ssg", "2_pn++"]:
                        # Get separate action/anchor embeddings
                        action_emb = self.emb_nn_action_at_goal(goal_action_points_dmean)
                        anchor_emb = self.emb_nn_anchor_at_goal(goal_anchor_points_dmean)
                    
                        # Apply cross-object transformer
                        if self.pzY_transformer in ["cross_object"]:
                            action_emb_tf, action_attn = self.emb_nn_action_at_goal.transformer(action_emb, anchor_emb)
                            anchor_emb_tf, anchor_attn = self.emb_nn_anchor_at_goal.transformer(anchor_emb, action_emb)
                ...
                if self.conditioning in ["hybrid_pos_delta_l2norm_global", "hybrid_pos_delta_l2norm_global_internalcond"]:
                ...
                if self.closest_point_conditioning is not None:
                    match = re.match(r"^top_([0-9]+)_(0\.[0-9]+)$", self.closest_point_conditioning)
                    ...
                    if np.random.rand() < prob:
                        ...
                        anchor_z_samples = []
                        for i in range(n_samples):
                            top_k_action_indices, top_k_anchor_indices, top_k_distances = sample_closest_pairs(
                                action_points.permute(0, 2, 1), 
                                anchor_points.permute(0, 2, 1), 
                                top_k=top_k
                            )
                            selected_action_indices, selected_anchor_indices, _ = sample_random_pair(
                                top_k_action_indices, 
                                top_k_anchor_indices, 
                                top_k_distances
                            )
                            
                            selected_action_onehots = torch.nn.functional.one_hot(
                                selected_action_indices.reshape(-1, 1), 
                                num_classes=action_points.shape[-1]
                            ).float().squeeze(1)
                            selected_anchor_onehots = torch.nn.functional.one_hot(
                ...
                with torch.set_grad_enabled(not self.freeze_z_embnn):
                    if self.pzY_encoder_type in ["pn++"]:
                        ...
                        for embedding_sample in embedding_samples:
                            ...
                            action_points_and_cond_xyz_centered = action_points_and_cond[:, :3] - action_points_and_cond[:, :3].mean(dim=2, keepdim=True)
                            anchor_points_and_cond_xyz_centered = anchor_points_and_cond[:, :3] - anchor_points_and_cond[:, :3].mean(dim=2, keepdim=True)
                            
                            # Concatenate the centered XYZ with the rest of the features
                            action_points_and_cond = torch.cat([action_points_and_cond_xyz_centered, action_points_and_cond[:, 3:]], axis=1)
                            anchor_points_and_cond = torch.cat([anchor_points_and_cond_xyz_centered, anchor_points_and_cond[:, 3:]], axis=1)
                            
                            cond_emb_points = torch.cat([action_points_and_cond, anchor_points_and_cond], axis=2)
                            ...
                            embedding_sample['for_debug']['goal_emb_logvar'] = goal_emb_logvar
                            embedding_sample['for_debug']['goal_emb_latent'] = goal_emb_latent

                    else:
                        raise ValueError(f"pzY_encoder_type {self.pzY_encoder_type} not implemented for embedding_routine {self.embedding_routine}")
        ...
        for embedding_sample in embedding_samples:
            ...
            elif self.conditioning in ["hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global_internalcond"]:
                tax_pose_conditioning_action = torch.tile(for_debug['goal_emb_latent'], (1, 1, action_points.shape[-1]))
                tax_pose_conditioning_anchor = torch.tile(for_debug['goal_emb_latent'], (1, 1, anchor_points.shape[-1]))
    
            # Prepare the action and anchor points for TAXPose
            if self.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_global"]:
                action_points_and_cond = torch.cat([
                    action_points_and_cond,
                    torch.tile(for_debug['goal_emb_latent'], (1, 1, action_points.shape[-1]))
                ], axis=1)
                anchor_points_and_cond = torch.cat([
                    anchor_points_and_cond,
                    torch.tile(for_debug['goal_emb_latent'], (1, 1, anchor_points.shape[-1]))
                ], axis=1)
    ...
    def sample_single(self, points, sample_type="action",**input_kwargs):
        if self.conditioning in ['uniform_prior_pos_delta_l2norm', 'uniform_prior_pos_delta_l2norm_dist_vec']:
        ...
        elif self.conditioning in ['hybrid_pos_delta_l2norm', 'hybrid_pos_delta_l2norm_internalcond', 'hybrid_pos_delta_l2norm_global', 'hybrid_pos_delta_l2norm_global_internalcond']:
            assert 'goal_emb' in input_kwargs, "goal_emb must be passed in for hybrid conditioning"
            assert 'sampling_method' in input_kwargs, "sampling_method must be passed in for hybrid conditioning"
            assert 'n_samples' in input_kwargs, "n_samples must be passed in for hybrid conditioning"
            ...
            for translation_sample in translation_samples:
                ...
                latent = torch.randn((points.shape[0], self.latent_z_linear_size//2, 1)).to(points.device)
                ...
                goal_emb_mu = torch.zeros((points.shape[0], 
                                                  self.latent_z_linear_size//2, 
                                                  points.shape[-1])).to(points.device)
                goal_emb_logvar = torch.ones((points.shape[0],
                                                    self.latent_z_linear_size//2,
                                                    points.shape[-1])).to(points.device)
                
                mu = torch.randn((points.shape[0], self.latent_z_linear_size//2, 1)).to(points.device)
                logvar = torch.randn((points.shape[0], self.latent_z_linear_size//2, 1)).to(points.device)
    ...
    def sample(self, action_points, anchor_points, **input_kwargs):
        if self.conditioning in ["latent_z_linear", "latent_z_linear_internalcond"]:
            ...
            for translation_sample_action, translation_sample_anchor in zip(translation_samples_action, translation_samples_anchor):
                # This is the only line that's different among the 3 different conditioning schemes in this category
                dense_trans_pt_action, ref_action = Multimodal_ResidualFlow_DiffEmbTransformer.get_dense_translation_point(None, 
                                                                                                                           action_points, 
                                                                                                                           translation_sample_action, 
                                                                                                                           conditioning='pos_delta_l2norm')
                dense_trans_pt_anchor, ref_anchor = Multimodal_ResidualFlow_DiffEmbTransformer.get_dense_translation_point(None, 
                                                                                                                           anchor_points, 
                                                                                                                           translation_sample_anchor, 
                                                                                                                           conditioning='pos_delta_l2norm')
                
                action_latent = torch.randn((action_points.shape[0], self.latent_z_linear_size//2, 1)).to(action_points.device)
                anchor_latent = torch.randn((anchor_points.shape[0], self.latent_z_linear_size//2, 1)).to(anchor_points.device)
                ...
                # TODO: Remove this, hacky way to be compatible w/ model without cond. x compute_loss() during pzX compute_loss()
                goal_emb_mu_action = torch.zeros((action_points.shape[0], 
                                                  self.latent_z_linear_size//2, 
                                                  action_points.shape[-1])).to(action_points.device)
                goal_emb_mu_anchor = torch.zeros((anchor_points.shape[0], 
                                                  self.latent_z_linear_size//2, 
                                                  anchor_points.shape[-1])).to(anchor_points.device)
                goal_emb_logvar_action = torch.ones((action_points.shape[0],
                                                    self.latent_z_linear_size//2,
                                                    action_points.shape[-1])).to(action_points.device)
                goal_emb_logvar_anchor = torch.ones((anchor_points.shape[0],
                                                    self.latent_z_linear_size//2,
                                                    anchor_points.shape[-1])).to(anchor_points.device)
                
                action_mu = torch.randn((action_points.shape[0], self.latent_z_linear_size//2, 1)).to(action_points.device)
                action_logvar = torch.randn((action_points.shape[0], self.latent_z_linear_size//2, 1)).to(action_points.device)
                anchor_mu = torch.randn((anchor_points.shape[0], self.latent_z_linear_size//2, 1)).to(anchor_points.device)
                anchor_logvar = torch.randn((anchor_points.shape[0], self.latent_z_linear_size//2, 1)).to(anchor_points.device)
                ...
                for_debug = {
                    ...
                    'goal_emb_logvar': torch.cat([action_logvar, anchor_logvar], axis=1),
                    'goal_emb_latent': goal_emb_latent,
                }
...
class Multimodal_ResidualFlow_DiffEmbTransformer_WithPZCondX(nn.Module):
    def __init__(self, residualflow_embnn, encoder_type="2_dgcnn", sample_z=True, 
                 ...
                 pzX_transformer="none", pzX_transformer_embnn_dims=512, pzX_transformer_emb_dims=512,
                 pzX_input_dims=3, pzX_dropout_goal_emb=0.0, hybrid_cond_pzX_sample_latent=False,
                 pzX_embedding_routine="joint", pzX_embedding_option=0):
        ...
        if self.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"] and self.hybrid_cond_pzX_sample_latent:
        ...
        if self.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond"]:
            if self.encoder_type == "1_dgcnn":
                ...
                self.p_z_cond_x_embnn = DGCNN(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, last_relu=False)
                ...
                args = VNArgs()
                self.p_z_cond_x_embnn_action = VN_DGCNN(args, num_part=self.emb_dims, gc=False)
                self.p_z_cond_x_embnn_anchor = VN_DGCNN(args, num_part=self.emb_dims, gc=False)
                ...
                self.p_z_cond_x_embnn_action = PointNet2MSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
                self.p_z_cond_x_embnn_anchor = PointNet2MSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            elif self.encoder_type == "2_pn++_ssg":
                print(f'--- P(z|X) Using 2 PN++ SSG ---')
                self.p_z_cond_x_embnn_action = PointNet2SSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
                self.p_z_cond_x_embnn_anchor = PointNet2SSG(num_classes=self.emb_dims, additional_channel=self.input_dims - 3)
            elif self.encoder_type == "2_pn++":
                print(f'--- P(z|X) Using 2 PyG PN++ ---')
                args = PN2DenseParams()
                self.p_z_cond_x_embnn_action = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.emb_dims, p=args)
                self.p_z_cond_x_embnn_anchor = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.emb_dims, p=args)
            else:
                raise ValueError()
        else:
            if self.encoder_type == "1_dgcnn":
                print(f'--- P(z|X) Using 1 DGCNN Classification ---')
                self.p_z_cond_x_embnn = DGCNNClassification(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads)
            elif self.encoder_type == "2_dgcnn":
                print(f'--- P(z|X) Using 2 DGCNN Classification ---')
                self.p_z_cond_x_embnn_action = DGCNNClassification(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, dropout=0.5, output_channels=self.latent_z_linear_size)
                self.p_z_cond_x_embnn_anchor = DGCNNClassification(input_dims=self.input_dims, emb_dims=self.emb_dims, num_heads=self.num_emb_heads, dropout=0.5, output_channels=self.latent_z_linear_size)
            ...
            if self.pzX_transformer == "cross_object":
                ...
                if self.conditioning not in ["latent_z_linear", "latent_z_linear_internalcond"]:
                    if self.encoder_type == "2_dgcnn":
                        ...
                        args = VNArgs()
                        self.p_z_cond_x_embnn_action = VN_DGCNN(args, num_part=self.pzX_transformer_embnn_dims, gc=False)
                        self.p_z_cond_x_embnn_anchor = VN_DGCNN(args, num_part=self.pzX_transformer_embnn_dims, gc=False)
                    elif self.encoder_type == "action_vndgcnn_anchor_dgcnn":
                        assert self.input_dims == 3, "Only support 3D input for VN_DGCNN"
                        print(f'------ With Action VN_DGCNN and Anchor DGCNN Encoders ------')
                        args = VNArgs()
                        self.p_z_cond_x_embnn_action = VN_DGCNN(args, num_part=self.pzX_transformer_embnn_dims, gc=False)
                        self.p_z_cond_x_embnn_anchor = DGCNN(input_dims=self.input_dims, emb_dims=self.pzX_transformer_embnn_dims, num_heads=1, last_relu=False)
                    elif self.encoder_type == "2_pn++_msg":
                        print(f'------ With 2 PN++ MSG Encoders ------')
                        self.p_z_cond_x_embnn_action = PointNet2MSG(num_classes=self.pzX_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                        self.p_z_cond_x_embnn_anchor = PointNet2MSG(num_classes=self.pzX_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                    elif self.encoder_type == "2_pn++_ssg":
                        print(f'------ With 2 PN++ SSG Encoders ------')
                        self.p_z_cond_x_embnn_action = PointNet2SSG(num_classes=self.pzX_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                        self.p_z_cond_x_embnn_anchor = PointNet2SSG(num_classes=self.pzX_transformer_embnn_dims, additional_channel=self.input_dims - 3)
                    elif self.encoder_type == "2_pn++":
                        print(f'------ With 2 PyG PN++ Encoders ------')
                        args = PN2DenseParams()
                        self.p_z_cond_x_embnn_action = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.pzX_transformer_embnn_dims, p=args)
                        self.p_z_cond_x_embnn_anchor = PN2DenseWrapper(in_channels=self.input_dims - 3, out_channels=self.pzX_transformer_embnn_dims, p=args)
                    else:
                        raise ValueError(f"pzX_transformer {self.pzX_transformer} not implemented for encoder_type {self.encoder_type}")
                elif self.conditioning in ["latent_z_linear", "latent_z_linear_internalcond"]:
                    if self.encoder_type == "1_dgcnn":
        ...
        if self.embedding_routine in ["action2anchor", "anchor2action", "action2anchor2global", "anchor2action2global"]:
            assert self.pzX_transformer != "none", "pzX_transformer must be set for action2anchor or anchor2action"
            ...
            if self.cond_embnn_type == "2_dgcnn":
                self.cond_embnn = DGCNN(input_dims=self.cond_input_dims, emb_dims=self.cond_emb_dims, num_heads=1, last_relu=False)
                pzX_net = DGCNN(input_dims=self.input_dims + self.emb_dims, emb_dims=self.pzX_transformer_embnn_dims, num_heads=1, last_relu=False)
            elif self.cond_embnn_type == "2_pn++":
                args = PN2DenseParams()
                self.cond_embnn = PN2DenseWrapper(in_channels=self.cond_input_dims - 3, out_channels=self.cond_emb_dims, p=args)
                pzX_net = PN2DenseWrapper(in_channels=self.input_dims + self.emb_dims - 3, out_channels=self.pzX_transformer_embnn_dims, p=args)
            else:
                raise ValueError(f"cond_embnn_type {self.cond_embnn_type} not implemented for action2anchor/anchor2action embedding")
            ...
            if self.embedding_routine in ["action2anchor2global", "anchor2action2global"]:
                self.global_mu = nn.Sequential(
                    PointNet([self.pzX_transformer_emb_dims, self.pzX_transformer_emb_dims//3, self.pzX_transformer_emb_dims//6]),
                    nn.Conv1d(self.pzX_transformer_emb_dims//6, self.latent_z_linear_size, kernel_size=1, bias=False),
                )
                ...
                # Use additional DGCNN + Transformer to get the global latent
                    ...
                    if self.cond_embnn_type == "2_dgcnn":
                        self.action_points_and_cond_embnn = DGCNN(input_dims=action_points_and_cond_emb_dims, emb_dims=self.pzX_transformer_emb_dims, num_heads=1, last_relu=False)
                        self.anchor_points_and_cond_embnn = DGCNN(input_dims=action_points_and_cond_emb_dims, emb_dims=self.pzX_transformer_emb_dims, num_heads=1, last_relu=False)
                    else:
                        raise ValueError(f"cond_embnn_type {self.cond_embnn_type} not implemented for embedding_option 1")
                    
                    self.action_global_emb_transformer = Transformer(emb_dims=self.pzX_transformer_emb_dims, return_attn=True, bidirectional=False)
                    self.anchor_global_emb_transformer = Transformer(emb_dims=self.pzX_transformer_emb_dims, return_attn=True, bidirectional=False)
                # Use TF embeddings and Cond. values + pointwise MLP to get the global latent
                    ...
                    self.action_emb_mlp = nn.Sequential(
                        ...
                        nn.Conv1d(mlp_dims, self.pzX_transformer_emb_dims, kernel_size=1, bias=False),
                    )
    ...
    def forward(self, *input, sampling_method="gumbel", n_samples=1, z_samples=None, sample_latent=False):
        action_points = input[0].permute(0, 2, 1)[:, :self.input_dims] # B,3,num_points
        anchor_points = input[1].permute(0, 2, 1)[:, :self.input_dims]
        ...
        if self.embedding_routine == "joint":
            ...
            if self.encoder_type == "1_dgcnn":
                ...
                # Concatenate the action and anchor embeddings
                if self.num_emb_heads > 1 and self.pzX_transformer not in ["cross_object"]:
                    goal_emb_cond_x = [
            ...
            if sample_latent:
                embedding_samples = Multimodal_ResidualFlow_DiffEmbTransformer.sample(self,
                                                                                    action_points[:, :3],
                                                                                    anchor_points[:, :3],
        ...
        # Get the action and anchor sequentially, action->anchor->goal emb. or anchor->action->goal emb.
        elif self.embedding_routine in ["action2anchor", "anchor2action", "action2anchor2global", "anchor2action2global"]:
            # Do action first
            if self.embedding_routine in ["action2anchor", "action2anchor2global"]:
                # Get embeddings
                ...
                first_emb_tf, first_emb_attn = self.p_z_cond_x_action_transformer(first_cond_emb, second_emb)
            ...
            # Do anchor first 
                ...
                first_emb_tf, first_emb_attn = self.p_z_cond_x_anchor_transformer(first_cond_emb, second_emb)
            ...
            if sample_latent:
                first_embedding_samples = Multimodal_ResidualFlow_DiffEmbTransformer.sample_single(
                    self,
                    points=action_points[:, :3] if self.embedding_routine in ["action2anchor", "action2anchor2global"] else anchor_points[:, :3],
                    goal_emb=first_emb,
                    sample_type="action" if self.embedding_routine in ["action2anchor", "action2anchor2global"] else "anchor",
                    sampling_method=sampling_method,
                    n_samples=n_samples
                )
            # Get spatially conditioned anchor
            else:
                first_embedding_samples = Multimodal_ResidualFlow_DiffEmbTransformer.add_single_conditioning(
                    self,
                    goal_emb=first_emb,
                    points=action_points[:, :3] if self.embedding_routine in ["action2anchor", "action2anchor2global"] else anchor_points[:, :3],
                    conditioning=self.conditioning if "global" not in self.embedding_routine else "pos_delta_l2norm",
                    cond_type="action" if self.embedding_routine in ["action2anchor", "action2anchor2global"] else "anchor",
                    sampling_method=sampling_method,
                    n_samples=n_samples,
                    z_samples=z_samples
                )
            ...
            cur_cond_type = "action" if self.embedding_routine in ["action2anchor", "action2anchor2global"] else "anchor"
            first_points_and_cond = first_embedding_samples[0][f'{cur_cond_type}_points_and_cond']
            first_points_and_cond_centered_xyz = first_points_and_cond[:, :3] - first_points_and_cond[:, :3].mean(dim=2, keepdim=True)
            first_points_and_cond_dmean = torch.cat([first_points_and_cond_centered_xyz, first_points_and_cond[:, 3:]], axis=1)
            ...
            if self.embedding_routine in ["action2anchor", "action2anchor2global"]:
                ...
                second_emb_tf, second_attn = self.p_z_cond_x_anchor_transformer(second_emb, first_emb2)
                second_emb = self.action_proj(second_emb_tf)
                
            else:
                # Get anchor embeddings
                first_emb2 = self.p_z_cond_x_embnn_anchor(first_points_and_cond_dmean)
                second_emb_tf, second_attn = self.p_z_cond_x_action_transformer(second_emb, first_emb2)
                second_emb = self.anchor_proj(second_emb_tf)
            
            # Get spatially conditioned second points
            if sample_latent:
                second_embedding_samples = Multimodal_ResidualFlow_DiffEmbTransformer.sample_single(
                    self,
                    points=anchor_points[:, :3] if self.embedding_routine in ["action2anchor", "action2anchor2global"] else action_points[:, :3], 
                    goal_emb=second_emb,
                    sample_type="anchor" if self.embedding_routine in ["action2anchor", "action2anchor2global"] else "action",
                    sampling_method=sampling_method,
                    n_samples=n_samples
                )
            else:
                second_embedding_samples = Multimodal_ResidualFlow_DiffEmbTransformer.add_single_conditioning(
                    self,
                    goal_emb=second_emb,
                    points=anchor_points[:, :3] if self.embedding_routine in ["action2anchor", "action2anchor2global"] else action_points[:, :3],
                    conditioning=self.conditioning if "global" not in self.embedding_routine else "pos_delta_l2norm",
                    cond_type="anchor" if self.embedding_routine in ["action2anchor", "action2anchor2global"] else "action",
                    sampling_method=sampling_method,
                    n_samples=n_samples,
                    z_samples=z_samples
                )
            ...
            if self.embedding_routine in ["action2anchor", "action2anchor2global"]:
                goal_emb_cond_x = torch.cat([prepare(first_emb, True), prepare(second_emb, False)], dim=-1)
                action_embedding_samples = first_embedding_samples
                anchor_embedding_samples = second_embedding_samples
            else:
                goal_emb_cond_x = torch.cat([prepare(second_emb, True), prepare(first_emb, False)], dim=-1)
            ...
            # Prepare outputs
            embedding_samples = []
            for action_embedding_sample, anchor_embedding_sample in zip(action_embedding_samples, anchor_embedding_samples):
                if self.conditioning in ["pos_delta_l2norm"]:
                    embedding_sample = {
                        'action_points_and_cond': action_embedding_sample['action_points_and_cond'],
                        'anchor_points_and_cond': anchor_embedding_sample['anchor_points_and_cond'],
                        'for_debug': {
                    ...
                    if not sample_latent and 'global' in self.embedding_routine:
                        # Just jointly predict the global latent from TF embeddings
                        if self.embedding_option == 0:
                            combined_emb = torch.cat([first_emb_tf, second_emb_tf], dim=-1)
                            
                            # Sum of average pool and max pool
                            global_emb = torch.mean(combined_emb, dim=2) + torch.max(combined_emb, dim=2)[0]
                        ...
                        # Use additional DGCNN + Transformer to get the global latent
                            ...
                            action_points_and_cond_emb = self.action_points_and_cond_embnn(action_points_and_cond)
                            anchor_points_and_cond_emb = self.anchor_points_and_cond_embnn(anchor_points_and_cond)
                            
                            action_points_and_cond_emb_tf, action_attn = self.action_global_emb_transformer(action_points_and_cond_emb, anchor_points_and_cond_emb)
                            anchor_points_and_cond_emb_tf, anchor_attn = self.anchor_global_emb_transformer(anchor_points_and_cond_emb, action_points_and_cond_emb)
                            
                            combined_emb = torch.cat([action_points_and_cond_emb_tf, anchor_points_and_cond_emb_tf], dim=-1)
                        ...
                        # Use TF embeddings and Cond. values + pointwise MLP to get the global latent
                            ...
                            action_cond_vals = action_embedding_sample['action_points_and_cond'][:, 3:]
                            anchor_cond_vals = anchor_embedding_sample['anchor_points_and_cond'][:, 3:]
                            
                            action_emb_tf = first_emb_tf if self.embedding_routine in ["action2anchor2global"] else second_emb_tf
                            anchor_emb_tf = second_emb_tf if self.embedding_routine in ["action2anchor2global"] else first_emb_tf

                            action_emb_tf_cond = torch.cat([action_emb_tf, action_cond_vals], dim=1)
                            anchor_emb_tf_cond = torch.cat([anchor_emb_tf, anchor_cond_vals], dim=1)
                            
                            action_emb_cond = self.action_emb_mlp(action_emb_tf_cond)
                            anchor_emb_cond = self.anchor_emb_mlp(anchor_emb_tf_cond)
                                                        
                            combined_emb = torch.cat([action_emb_cond, anchor_emb_cond], dim=-1)
                    ...
                    # Otherwise use the sampled latents, or independent action/anchor latents (non global)
                        ...
                        embedding_sample['for_debug']['goal_emb_mu'] = torch.cat([action_embedding_sample['for_debug']['action_mu'], anchor_embedding_sample['for_debug']['anchor_mu']], axis=1)
                        embedding_sample['for_debug']['goal_emb_logvar'] = torch.cat([action_embedding_sample['for_debug']['action_logvar'], anchor_embedding_sample['for_debug']['anchor_logvar']], axis=1)
                        embedding_sample['for_debug']['goal_emb_latent'] = torch.cat([action_embedding_sample['for_debug']['action_latent'], anchor_embedding_sample['for_debug']['anchor_latent']], axis=1)
        ...
        for embedding_sample in embedding_samples:
            ...
            if self.conditioning == "latent_z_linear_internalcond":
                tax_pose_conditioning_action = torch.tile(for_debug['goal_emb_sample'], (1, 1, action_points.shape[-1]))
                tax_pose_conditioning_anchor = torch.tile(for_debug['goal_emb_sample'], (1, 1, anchor_points.shape[-1]))
            if self.conditioning in ["hybrid_pos_delta_l2norm_internalcond", "hybrid_pos_delta_l2norm_global_internalcond"]:
                tax_pose_conditioning_action = torch.tile(for_debug['goal_emb_latent'], (1, 1, action_points.shape[-1]))
                tax_pose_conditioning_anchor = torch.tile(for_debug['goal_emb_latent'], (1, 1, anchor_points.shape[-1]))
            ...
            if input[2] is not None:
                ...
                if self.conditioning in ["hybrid_pos_delta_l2norm", "hybrid_pos_delta_l2norm_internalcond"]:
                    pzY_logging['pzY_goal_emb_mu_action'] = pzY_results[0]['goal_emb_mu_action']
                    pzY_logging['pzY_goal_emb_mu_anchor'] = pzY_results[0]['goal_emb_mu_anchor']
                    pzY_logging['pzY_goal_emb_logvar_action'] = pzY_results[0]['goal_emb_logvar_action']
                    pzY_logging['pzY_goal_emb_logvar_anchor'] = pzY_results[0]['goal_emb_logvar_anchor']
                ...
                # Pass through additional logging
                    ...
                    pzY_logging['pzY_goal_emb_logvar'] = pzY_results[0]['goal_emb_logvar']
                elif self.conditioning in ["latent_z_linear", "latent_z_linear_internalcond"]:
                    pzY_logging['pzY_goal_emb_mu'] = pzY_results[0]['goal_emb_mu']
                    pzY_logging['pzY_goal_emb_logvar'] = pzY_results[0]['goal_emb_logvar']
