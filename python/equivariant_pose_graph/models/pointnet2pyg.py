...
class PN2DenseWrapper(nn.Module):
    def __init__(self, in_channels: int = 0, out_channels: int = 3, p: PN2DenseParams = PN2DenseParams()):
    ...
    def forward(self, x: torch.Tensor, conditioning: torch.Tensor = None):
        ...
        data_pos = x.permute(0, 2, 1).reshape(batch_size * num_points, feature_dim).to(x.device)
        data_x = None
        if feature_dim > 3:
            data_x = data_pos[:, 3:]
            data_pos = data_pos[:, :3]
...
class PN2EncoderWrapper(nn.Module):
    def __init__(self, in_channels: int = 0, out_channels: int = 1024, num_heads: int = 1, emb_dims: int = 512, p: PN2EncoderParams = PN2EncoderParams()):
        ...
        if self.num_heads == 1:
            ...
            self.linear = nn.ModuleList([nn.Linear(emb_dims, out_channels) for _ in range(num_heads)])
...
class PN2HybridWrapper(nn.Module):
    def __init__(self, in_channels: int = 0, out_channels: int = 3, num_heads: int = 2, emb_dims: int = 512, p: PN2DenseParams = PN2DenseParams()):
        ...
        if num_heads == 1:
            ...
            self.lin3 = nn.ModuleList([nn.Linear(p.lin2_dim, out_channels) for _ in range(num_heads)])
    ...
    def forward(self, x: torch.Tensor, conditioning: torch.Tensor = None):
        ...
        if self.num_heads == 1:
            ...
            out_int = [linear(out_int).reshape(batch_size, -1, 1) for linear in self.lin3]
