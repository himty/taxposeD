...
class ResidualMLPHead(nn.Module):
    ...
    def __init__(self, emb_dims=512, output_dims=3, pred_weight=True, residual_on=True, 
                 conditioning_type='old', conditioning_size=0, use_weighted_sum=True, use_selected_point_feature=False):
        super(ResidualMLPHead, self).__init__()
        self.flow_emb_dims = emb_dims
        self.weight_emb_dims = emb_dims
        self.conditioning_type = conditioning_type
        
        self.use_weighted_avg = use_weighted_sum
        self.use_selected_point_feature = use_selected_point_feature
        self.emb_mul = 1
        if self.use_selected_point_feature:
            self.emb_mul = 2
    ...
    def forward(self, *input, scores=None, return_flow_component=False, return_embedding=False, conditioning=None):
        ...
        if self.use_weighted_avg:
            ...
            corr_points_onehot = F.gumbel_softmax(scores.transpose(2, 1), tau=1, hard=True, dim=1)
            corr_points = torch.matmul(anchor_points, corr_points_onehot)
        ...
        if self.use_selected_point_feature:
            selected_anchor_embeddings = torch.matmul(anchor_embedding, corr_points_onehot)            
            embedding = torch.cat([action_embedding, selected_anchor_embeddings], dim=1)
...
class ResidualFlow_DiffEmbTransformer(nn.Module):
    def __init__(self, emb_dims=512, input_dims=3, cycle=True, emb_nn='dgcnn', return_flow_component=False, center_feature=False,
                 ...
                 conditioning_type='old', n_heads=4, flow_head_use_weighted_sum=True, flow_head_use_selected_point_feature=False,
                 post_encoder_input_dims=3, flow_direction="both"):
