...
def sample_uniform_cube(radius, num_dim_points):
    ...
    point_coords = np.stack([point_coords[0].flatten(), point_coords[1].flatten(), point_coords[2].flatten()], axis=1)
...
def sample_closest_pairs(action_points, anchor_points, top_k=50):
    ...
    squared_distances = ((action_points[:, :, None, :3] - anchor_points[:, None, :, :3])**2).sum(dim=-1)
    
    # Find top_k closest pairs for each batch
    top_k_distances, flat_indices = torch.topk(squared_distances.view(squared_distances.shape[0], -1), top_k, dim=-1, largest=False, sorted=True)
...
def sample_random_pair(top_k_action_indices, top_k_anchor_indices, top_k_distances):
    ...
    selected_action_indices = top_k_action_indices[torch.arange(batch_size), selected_indices]
    selected_anchor_indices = top_k_anchor_indices[torch.arange(batch_size), selected_indices]
