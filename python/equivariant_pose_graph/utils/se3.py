...
# TODO REMEMBER THIS CHANGED uniform rotations (rot_var == np.pi) ON 9/6/23 at 5pm EST
...
def get_degree_angle(T):
    angle_rad_T = so3_rotation_angle(
        T.get_matrix()[:, :3, :3], eps=1e-2)*180/np.pi  # B
...
def get_2transform_min_rotation_errors(T1, T2):
    angle_rad_T1 = so3_rotation_angle(
        T1.get_matrix()[:, :3, :3], eps=1e-2)*180/np.pi  # B
    angle_rad_T2 = so3_rotation_angle(
        T2.get_matrix()[:, :3, :3], eps=1e-2)*180/np.pi  # B
...
def flow2pose(xyz, flow, weights=None, return_transform3d=False, normalization_scehme='l1', x=None, temperature=1):
    """
    ...
    X = torch.bmm(xyz_demean.transpose(-2, -1),
                  w*xyz_trans)
...
def dualflow2pose(xyz_src, xyz_tgt,
                  flow_src, flow_tgt,
                  weights_src=None, weights_tgt=None,
    ...
    assert torch.allclose(w_src.sum(1), torch.ones(w_src.sum(1).shape).cuda(
    )), "flow src weights does not sum to 1 for each batch element"
    assert torch.allclose(w_tgt.sum(1), torch.ones(w_tgt.sum(1).shape).cuda(
    ...
    t = ((w_src.shape[1] * t_src + w_tgt.shape[1] * t_tgt) /
         (w_src.shape[1] + w_tgt.shape[1])).squeeze(1)
...
def dualflow2translation(xyz_src, xyz_tgt,
                         flow_src, flow_tgt,
                         weights_src=None, weights_tgt=None,
                         return_transform3d=False,
                         normalization_scehme='l1',
...
def dualflow2pose_joint(xyz, flow, polarity, weights=None, return_transform3d=False, normalization_scehme='l1'):
    assert normalization_scehme in [
        'l1', 'softmax'], "normalization_scehme: {} is not currently supported!".format(normalization_scehme)
    if(weights is None):
        weights = torch.ones(xyz.shape[: -1], device=xyz.device)
...
def points2pose(xyz1, xyz2, weights=None, return_transform3d=False, normalization_scehme='l1'):
    assert normalization_scehme in [
        'l1', 'softmax'], "normalization_scehme: {} is not currently supported!".format(normalization_scehme)
    if(weights is None):
...
def consistency_flow_loss(points, flow_pred, weights_pred=None):
    T_pred = flow2pose(points, flow_pred, weights_pred,
                       return_transform3d=True).detach()
    points_pred = T_pred.transform_points(points)
...
def consistency_dualflow_loss(points_trans_action, points_trans_anchor, pred_flow_action, pred_flow_anchor, pred_w_action, pred_w_anchor, weight_normalize='softmax'):
    pred_T_action = dualflow2pose(xyz_src=points_trans_action, xyz_tgt=points_trans_anchor,
                                  ...
                                  return_transform3d=True, normalization_scehme=weight_normalize)
