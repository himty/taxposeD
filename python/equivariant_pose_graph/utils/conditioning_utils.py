...
def gumbel_softmax_topk(logits: torch.Tensor, k: int, tau: float, hard: bool = False, dim: int = -1) -> Tuple[torch.Tensor, List[torch.Tensor]]:
    """
    ...
    """
    gumbel = torch.distributions.gumbel.Gumbel(torch.zeros_like(logits), torch.ones_like(logits))
    g = gumbel.sample()
    y = logits + g
    ...
    for i in range(k):
        khot_mask = torch.max(1.0 - onehot_approx, torch.tensor([np.finfo(np.float32).tiny], device=y.device))
