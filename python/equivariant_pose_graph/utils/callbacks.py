...
class SaverCallbackModel(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx): #, dataloader_idx):
        ...
        if global_step % self.save_freq == 0 and global_step > 100:
            ...
            torch.save(
                {'state_dict': pl_module.state_dict()}, ckpt_path_embnn)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx): #, dataloader_idx):
        ...
        if global_step % self.save_freq == 0:
            ...
            if not os.path.isdir(trainer.checkpoint_callback.dirpath):
                os.makedirs(trainer.checkpoint_callback.dirpath)
            torch.save(
                {'embnn_state_dict': pl_module.model.emb_nn.state_dict()}, ckpt_path_embnn)
            if(self.prev_path is not None):
                self.prev_path.unlink()
                self.prev_path = pathlib.Path(ckpt_path_embnn)
        ...
        if global_step % self.save_freq == 0:
            ...
            torch.save(
                {'embnn_action_state_dict': pl_module.model.emb_nn_action.state_dict(),
                 'embnn_anchor_state_dict': pl_module.model.emb_nn_anchor.state_dict()}, ckpt_path_embnn)
            if(self.prev_path is not None):
                self.prev_path.unlink()
                self.prev_path = pathlib.Path(ckpt_path_embnn)


class SaverCallbackEmbnnActionAnchorMultimodal(Callback):
    """
    Save a checkpoint every N steps, instead of Lightning's default that checkpoints
    based on validation loss.
    """
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx): #, dataloader_idx):
        ...
        if global_step % self.save_freq == 0:
            ...
            if not os.path.isdir(trainer.checkpoint_callback.dirpath):
                os.makedirs(trainer.checkpoint_callback.dirpath)
            # TODO put in a more general fix
            torch.save(
                {'embnn_objs_at_goal_state_dict': pl_module.model.emb_nn_objs_at_goal.state_dict(),
                 'taxpose_embnn_action_state_dict': pl_module.model.tax_pose.emb_nn_action.state_dict(),
                 'taxpose_embnn_anchor_state_dict': pl_module.model.tax_pose.emb_nn_anchor.state_dict()}, ckpt_path_embnn)
            if(self.prev_path is not None):
                self.prev_path.unlink()
                self.prev_path = pathlib.Path(ckpt_path_embnn)
...
class SaverCallbackRefinement(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx): #, dataloader_idx):
        ...
        global_step = trainer.global_step
        if global_step % self.save_freq == 0:
            filename = f"refinement_callback_epoch_{epoch}_global_step_{global_step}.ckpt"
            ckpt_path_embnn = os.path.join(
                trainer.checkpoint_callback.dirpath, filename)
            if not os.path.isdir(trainer.checkpoint_callback.dirpath):
                os.mkdir(trainer.checkpoint_callback.dirpath)
            torch.save(
                {'refinement_state_dict': pl_module.refinement_model.state_dict()}, ckpt_path_embnn)
            if(self.prev_path is not None):
                self.prev_path.unlink()
                self.prev_path = pathlib.Path(ckpt_path_embnn)
