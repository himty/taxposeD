...
def get_rpdiff_min_errors(pred_T_action, batch, trans_thresh=0.02, rpdiff_descriptions_path=None):
    ...
    if 'scene_rack_transforms' in batch:
        ...
        batch_mug_final_poses_list = batch_mug_final_poses.permute(1, 0).unsqueeze(1) # Match rack pose dims

        # Get each batch idx's rack transforms
        batch_rack_transforms = torch.stack(batch['scene_rack_transforms']['racks']).permute(1, 0, 2, 3, 4)
        batch_rack_transforms_list = []
        for batch_idx in range(len(batch_rack_transforms)):
            nans = torch.all(torch.all(torch.all(batch_rack_transforms[batch_idx].isnan(), dim=1) == True, dim=1) == True, dim=1)
        ...
        for batch_idx in range(len(batch_mug_ids_list)):
            ...
            for rack in batch_rack_ids:
                ...
                rack_avail_mug_poses_dir = osp.join(rpdiff_descriptions_path, f'objects/{rack_type}_unnormalized/available_mug_poses/{rack}')
                cur_mug_poses = np.load(osp.join(rack_avail_mug_poses_dir, f'{batch_mug_id}/mug_poses.npz'), allow_pickle=True)
        ...
        for batch_idx in range(len(batch_mug_ids_list)):
            ...
            for rack_idx in range(len(batch_avail_mug_poses[batch_idx])):
                ...
                avail_mug_poses_tf = Transform3d(matrix=torch.from_numpy(cur_rack_avail_poses).float().permute(0, 2, 1)).to(batch["points_action"].device)
                
                # Get the transform that moves each rack from the origin to its scene pose
                scene_rack_transforms = Transform3d(matrix=batch_rack_transforms_list[batch_idx][rack_idx].T).to(batch["points_action"].device)
                
                # Get the available poses at the scene rack poses
                avail_mug_poses_scene = avail_mug_poses_tf.compose(scene_rack_transforms)
                
                # Center the available poses about the action mean
                scene_to_action_mean = Translate(-batch['points_action_mean'][batch_idx].unsqueeze(0)).to(batch["points_action"].device)
                avail_mug_poses_scene_centered = avail_mug_poses_scene.compose(scene_to_action_mean)
                
                # Move the available poses to the anchor frame
                avail_poses_origin_to_anchor = avail_mug_poses_scene_centered.compose(T1[batch_idx])
                ...
                translate_to_action_mean = Translate(batch['points_action_mean'][batch_idx].unsqueeze(0)).to(batch["points_action"].device)
                demo_to_action_mean_tf = transformed_to_demo_tf.compose(translate_to_action_mean)
                
                # Undo rack scene transform
                demo_rack_scene_transform = Transform3d(matrix=batch_rack_transforms_list[batch_idx][batch_demo_rack_idx[batch_idx]].T).to(batch["points_action"].device)
                action_mean_to_scene_tf = demo_to_action_mean_tf.compose(demo_rack_scene_transform.inverse())
                
                # Move to demo final rack pose
                demo_rack_final_pose = batch_rack_final_poses_list[batch_idx][batch_demo_rack_idx[batch_idx]]
                demo_rack_final_pose_mat = matrix_from_list(demo_rack_final_pose.detach().cpu().tolist())
                demo_rack_final_pose_tf = Transform3d(matrix=torch.Tensor(demo_rack_final_pose_mat).T).to(batch["points_action"].device)
                scene_to_demo_rack_final_pose_tf = action_mean_to_scene_tf.compose(demo_rack_final_pose_tf)
                
                # Move to mug origin
                cur_mug_final_pose = batch_mug_final_poses_list[batch_idx]
                cur_mug_final_pose_mat = matrix_from_list(cur_mug_final_pose[0].detach().cpu().tolist())
                cur_mug_final_pose_tf = Transform3d(matrix=torch.Tensor(cur_mug_final_pose_mat).T).to(batch["points_action"].device)
                demo_rack_final_pose_to_mug_origin_tf = scene_to_demo_rack_final_pose_tf.compose(cur_mug_final_pose_tf.inverse())
                
                # Move action points to available poses
                transformed_action_to_avail_poses_tf = demo_rack_final_pose_to_mug_origin_tf.compose(avail_poses_origin_to_anchor)
                
                cur_batch_avail_mug_poses_comparable.append(transformed_action_to_avail_poses_tf)
            batch_avail_mug_poses_comparable.append(cur_batch_avail_mug_poses_comparable)

        batch_min_dists = []
        for batch_idx in range(len(batch_mug_ids_list)):
            pred_pose_mat = pred_T_action[batch_idx].get_matrix().squeeze(0).T.detach().cpu().numpy()
            ...
            for rack_poses in batch_avail_mug_poses_comparable[batch_idx]:
                rack_poses_mat = rack_poses.get_matrix().permute(0, 2, 1).detach().cpu().numpy()
                for pose in rack_poses_mat:
                    ...
                    rot_pred = Rotate(torch.Tensor(pred_pose_rot.T)).to(batch["points_action"].device)
                    rot_pose = Rotate(torch.Tensor(pose_rot.T)).to(batch["points_action"].device)
    ...
    # We are doing the initial WTA implementation for book or can
        ...
        for batch_idx in range(len(parent_fnames)):
            ...
            for pose in avail_poses_trans_anchor_frame:
                ...
                rot_child_pred = Rotate(torch.Tensor(child_pred_pose_rot.T)).to(batch["points_action"].device)
                rot_pose = Rotate(torch.Tensor(pose_rot.T)).to(batch["points_action"].device)
