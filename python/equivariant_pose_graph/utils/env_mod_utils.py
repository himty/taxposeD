...
def random_transform_rack(points_anchor, rot_var=np.pi, trans_var=1, rot_sample_method="axis_angle"):
    ...
    # Assume the random_se3() function is uniform random in SE(3)..
...
def get_random_rack_demo(points_gripper, points_action, points_anchor_base, rot_var=np.pi, trans_var=1, return_debug=False, rot_sample_method="axis_angle", num_racks_to_add=1):
    debug = {}
    ...
    for i in range(num_racks_to_add):
        ...
        # Update rect_prisms_base to also use the new points_anchor2
        rect_prisms_base = combine_axis_aligned_rect([
            rect_prisms_base,
