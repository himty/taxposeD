...
def combine_axis_aligned_rect(rect_prisms):
    ...
    return torch.hstack([torch.min(torch.stack([r[:, 0:3] for r in rect_prisms], dim=0), dim=0)[0],
...
def axis_aligned_rect_intersect(rect_prism1, rect_prism2):
    ...
    conditions = (rect_prism1[:,0] <= rect_prism2[:,3]).int() \
...
def random_transform_rack(points_anchor, rot_var=np.pi, trans_var=1, rot_sample_method="axis_angle"):
    ...
    T = random_se3(N, rot_var=rot_var, trans_var=trans_var, device=device, rot_sample_method=rot_sample_method)
    return T.transform_points(points_anchor), T
    
def get_nonintersecting_rack(points_anchor_base, rect_prisms_base, rot_var=np.pi, trans_var=1, return_debug=False, rot_sample_method="axis_angle"):
    points_anchor1 = points_anchor_base
    rect_prisms1 = rect_prisms_base
    
    success = torch.tensor([False]*points_anchor1.shape[0], device=points_anchor1.device)
    tries = 0
    ...
    while not torch.all(success): # and tries < 10:
        points_anchor2_temp, T_temp = random_transform_rack(points_anchor_base, rot_var=rot_var, trans_var=trans_var, rot_sample_method=rot_sample_method)
        ...
        points_anchor2[intersects_temp == False] = points_anchor2_temp[intersects_temp == False]
        # This doesnt work:
        T = T_temp
...
def get_random_rack_demo(points_gripper, points_action, points_anchor_base, rot_var=np.pi, trans_var=1, transform_base=True, return_debug=False, rot_sample_method="axis_angle", num_racks_to_add=1):
    debug = {}
    if transform_base:
        N = points_anchor_base.shape[0]
        T = random_se3(N, rot_var=rot_var, trans_var=trans_var, device=points_anchor_base.device, fix_random=False)
    ...
    rect_prisms_base = combine_axis_aligned_rect([
        ...
        ] if points_gripper is not None else [])
    )
    
    new_points_list = []
    T_aug_list = []
    
    for i in range(num_racks_to_add):
        points_anchor2, T2, debug_temp = get_nonintersecting_rack(points_anchor_base, rect_prisms_base, rot_var=rot_var, trans_var=trans_var, return_debug=return_debug, rot_sample_method=rot_sample_method)
        ...
        if return_debug:
            ...
            rect_prisms2 = points_to_axis_aligned_rect(points_anchor2)
            debug_temp = dict(tries=debug['tries'], rect_prisms1=rect_prisms1, rect_prisms2=rect_prisms2)
            debug.update(debug_temp)
            
        new_points_list.append(points_anchor2)
        T_aug_list.append(T2)
        
        # Update rect_prisms_base to also include the new rack
        rect_prisms_base = combine_axis_aligned_rect([
            rect_prisms_base,
            points_to_axis_aligned_rect(points_anchor2),
        ])

    return points_gripper, points_action, points_anchor1, new_points_list, T_aug_list, debug
...
class MugOnRackPerturbs():
    ...
    def duplicate_k_rack_table(
        ...
        all_rack_transforms = random_se3(ks.sum(), rot_var=rot_var,
                            trans_var=trans_var, device=mug_demo_pts.device)
        ...
        for i, k in enumerate(ks):
            ...
            # Create the racks
            all_rack_pts[i] = my_transforms.transform_points(rack_demo_pts[i]).reshape(-1, 3)
            ...
            all_mug_pts[i] = my_transforms[rack_i].transform_points(mug_demo_pts[i].float())
    ...
    def duplicate_k_rack(
        ...
        rack_demo_pts, ids = sample_farthest_points(rack_demo_pts,
                                 K=points_per_cloud, random_start_point=random_start_point)
