...
def InfoNCETraining(model, points, transform, temperature=0.1):
    ...
    phi_trans = model(points_trans_centered.transpose(1, 2)
                      )  # B, emb_dim, num_points

    similarity = phi.transpose(-1, -2) @ phi_trans

    target = torch.arange(
        similarity.shape[-1], device=similarity.device).tile([similarity.shape[0], 1])  # B, num_points
...
def compute_infonce_loss(model: nn.Module, points: torch.Tensor, 
                         normalize_features: bool = True, weighting: str = 'dist', 
                         l2_reg_weight: float = 0.0, center=None, 
                         log_prefix: str = '') -> Tuple[torch.Tensor, Dict]:
    """
    ...
    if center is None:
        points_centered_xyz = points[:, :, :3] - points[:, :, :3].mean(dim=1, keepdims=True)
        ...
        points_action_rotated_centered_xyz = points_action_rotated_xyz - \
            points_action_rotated_xyz.mean(dim=1, keepdims=True)
        points_action_rotated_centered = torch.cat([points_action_rotated_centered_xyz, points[:, :, 3:]], dim=-1)
    else:
    ...
    if l2_reg_weight > 0:
        ...
        l2_reg = mse_criterion(phi_norm, torch.zeros_like(phi_norm)) \
            + mse_criterion(phi_rotated_norm,
                            torch.zeros_like(phi_rotated_norm))
...
def compute_occlusion_infonce_loss(model: nn.Module, points: torch.Tensor, 
                         normalize_features: bool = True, weighting: str = 'dist', 
                         l2_reg_weight: float = 0.0, center=None, 
                         log_prefix: str = '') -> Tuple[torch.Tensor, Dict]:
    """
    ...
    for batch_idx in range(points.shape[0]):
        ...
        while max_attempts > 0:
            ...
            if np.random.rand() > 0.5:
                points_occ_xyz, mask = ball_occlusion(points[batch_idx, :, :3], return_mask=True)
            else:
                points_occ_xyz, mask = plane_occlusion(points[batch_idx, :, :3], return_mask=True)
            points_occ = torch.cat([points_occ_xyz, points[batch_idx, mask, 3:]], dim=-1)
    ...
    for batch_idx, (phi_occ, mask) in enumerate(zip(phi_occ_list, mask_list)):
        ...
        mean_order_error = mean_order(similarity)
        mean_geo_error = mean_geo_diff(similarity, points[batch_idx, mask, :3].unsqueeze(0))
